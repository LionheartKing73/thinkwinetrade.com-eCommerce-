...
class ControllerApiOrder extends Controller {
	...
	public function history() {
		...
		if (!isset($this->session->data['api_id'])) {
			...
			if ($order_info) {
				if($this->request->post['send_po']) { 
					$this->request->post['order_status_id'] = $this->config->get('po_status_waiting_po_confirmation_id');

					$this->load->model('checkout/po');

					$this->model_checkout_po->sendPo($order_id, $this->request->post['order_status_id']);					
				}
				elseif($this->request->post['send_reception_product']) {
                                $this->request->post['order_status_id'] = $this->config->get('po_status_waiting_po_confirmation_id');

					$this->load->model('checkout/po');

					$this->model_checkout_po->sendNotifProduct($order_id, 2);					
				}
                                if(!$this->request->post['send_reception_product']) {
                                 
                $this->model_checkout_order->addOrderHistory($order_id, $this->request->post['order_status_id'], urldecode($this->request->post['comment']), $this->request->post['notify'], $this->request->post['sent_comment_to_all']);
