...
class ControllerUserUser extends Controller {
	...
	protected function getForm() {
        ...
        $column_exists = $this->db->query("SHOW COLUMNS FROM `" . DB_PREFIX . "user` LIKE 'language_id'");
        if (!$column_exists->num_rows) {
            $this->db->query("ALTER TABLE `" . DB_PREFIX . "user` ADD COLUMN `language_id` INT(11) NOT NULL DEFAULT '1' AFTER `user_group_id`");
        }
        ...
        if (isset($this->request->post['language_id'])) {
            ...
            $data['language_id'] = $user_info['language_id'];
        } else {
