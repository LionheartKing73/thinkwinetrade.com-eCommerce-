...
'worksheet_id'            => $worksheet_id,
				...
				'email_review'			  => $order_query->row['email_review'],
  ...
  public function getVendorsFobTotals($order_id){
	...
	public function getOrders($data = array()) {
		$sql = "SELECT (SELECT SUM(op.fob_bottle*op.fob_case*op.quantity) FROM ". DB_PREFIX ."order_product op WHERE op.order_id=o.order_id) as fob_total, o.order_id, CONCAT(o.firstname, ' ', o.lastname) AS customer, (SELECT COUNT(p.pallet_id) FROM " . DB_PREFIX . "pallet p WHERE p.order_id = o.order_id) AS noofpallets, (SELECT os.name FROM " . DB_PREFIX . "order_status os WHERE os.order_status_id = o.order_status_id AND os.language_id = '" . (int)$this->config->get('config_language_id') . "') AS status, o.shipping_code, o.order_status_id, o.total, o.currency_code, o.currency_value, o.date_added, o.date_modified FROM `" . DB_PREFIX . "order` o";
...
AND o.payment_status = '5'";
	...
	public function getTotalPallet( $order_id ) {
			$query = $this->db->query("SELECT  COUNT(*) AS `total` FROM " . DB_PREFIX . "pallet p  WHERE  p.order_id= '" . (int)$order_id . "' " );
			$total = 0;
			if ( $query->num_rows ) {
	...
	public function confirmProductReceived($order_id, $product_id, $pallet_id) {
		$this->db->query("UPDATE `" . DB_PREFIX . "pallet_product` SET product_received = '1', product_received_date = NOW()  WHERE order_id = '" . (int)$order_id . "' AND product_id = '" . (int)$product_id . "' AND pallet_id = '" . (int)$pallet_id . "'");
	}

	public function confirmDocumentsReceived($order_id, $product_id, $pallet_id) {
		$this->db->query("UPDATE `" . DB_PREFIX . "pallet_product` SET documents_received = '1', documents_received_date = NOW() WHERE order_id = '" . (int)$order_id . "' AND product_id = '" . (int)$product_id . "' AND pallet_id = '" . (int)$pallet_id . "'");
	}

	public function adminConfirmedReceived($order_id) {
		$query = $this->db->query("SELECT COUNT(*) as total FROM `" . DB_PREFIX . "pallet_product` WHERE order_id = '" . (int)$order_id . "' AND (product_received = '0' OR documents_received = '0')");
    ...
    public function getFobPrices($order_id, $product_id){
        $query = $this->db->query("SELECT fob_bottle, fob_case, fob_bottle_full_price FROM " . DB_PREFIX . "order_product
             WHERE order_id = '" . (int)$order_id . "' and product_id=".$product_id);
       
        return $query->row;
    }
	
	public function update_email_review($order_id){
		$this->db->query("UPDATE " . DB_PREFIX . "order SET email_review = '1' WHERE order_id = '" . (int)$order_id . "'");
	}
	
	
	public function getTotalProductsInOrders($data = array()) {
...
if (!empty($data['filter_pallet_id'])) {
	...
	//END
		...
		if (count($query->rows) ){
			 foreach($query->rows as $row){
				  ...
				  $q = $this->db->query('SELECT COUNT(p.pallet_id) as total FROM ' . DB_PREFIX . 'pallet p WHERE p.order_id='.$row['order_id']);
    ...
    public function getContainers($data) {
		$sql = "SELECT distinct container_id as name FROM `" . DB_PREFIX . "order` where container_id like '%".$data['filter_container_id']."%' limit ".$data['start'].','.$data['limit'];
	 ...
	 public function getShipment($data) {
		$sql = "SELECT distinct shipment_id as name FROM `" . DB_PREFIX . "order` where shipment_id like '%".$data['filter_shipment_id']."%' limit ".$data['start'].','.$data['limit'];
