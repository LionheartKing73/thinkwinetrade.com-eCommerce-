<?php
class ModelCatalogVDICategory extends Model {
	public function addCategory($data) {
		$this->event->trigger('pre.admin.add.category', $data);
		
		if ($this->config->get('mvd_menu_bar')) {
	...
	public function editCategory($category_id, $data) {
		...
		if ($this->config->get('mvd_menu_bar')) {
			...
			$this->db->query("UPDATE " . DB_PREFIX . "category SET parent_id = '" . (int)$data['parent_id'] . "', `column` = '" . (int)$data['column'] . "', sort_order = '" . (int)$data['sort_order'] . "', status = '" . (int)$data['status'] . "', date_modified = NOW() WHERE category_id = '" . (int)$category_id . "'");
		}
	...
	public function getCategories($data = array()) {
		...
		$ucats = array();
		$ucats = unserialize($this->user->getCP());
		
		if ($ucats) {
			$urange = implode(',',unserialize($this->user->getCP()));
		$sql .= " AND cp.category_id IN($urange)";	
		}
	...
	public function getTotalCategories() {
		...
		if ($ucats) {
			$urange = implode(',',unserialize($this->user->getCP()));
			$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "category WHERE category_id IN($urange)");
		} else {
	...
	/*code start*/
	public function getVendorCategories($parent_id = 0) {
