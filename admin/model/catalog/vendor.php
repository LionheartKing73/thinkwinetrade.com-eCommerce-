...
// Load Vendor Language File MH 2017-02-07
            $langquery = $this->db->query("SELECT * FROM `" . DB_PREFIX . "language` WHERE language_id IN (SELECT `language_id` FROM `" . DB_PREFIX . "user` WHERE `user_id` = '" . (int)$user_id . "');");
...
// End Load Vendor Language File
			...
			$language->load('mail/signup');
			$mdata['title'] = sprintf($language->get('text_subject'), $this->config->get('config_name'));
			
			$mdata['text_message'] = sprintf($language->get('text_dear'), $data['firstname'] . ' ' . $data['lastname']) . ",<br /><br />";			
			$mdata['text_message'] .= sprintf($language->get('text_welcome1'), $this->config->get('config_name')) . "<br /><br />";
			$mdata['text_message'] .= $language->get('text_login') . "<br /><br />";
			
			if ($data['password']) {
				$mdata['text_message'] .= $language->get('text_info') . "<br />";
				...
				$mdata['text_message'] .= $language->get('text_username') . $username . "<br />";
				$mdata['text_message'] .= $language->get('text_password') . $data['password'] . "<br />";
			}
			
			$mdata['text_message'] .= $language->get('text_url') . HTTP_SERVER . "<br /><br />";
			$mdata['text_message'] .= $language->get('text_services') . "<br /><br />";

			$mdata['text_message'] .= $language->get('text_thanks') . "<br />";
...
// Load Vendor Language File MH 2017-02-07
            $langquery = $this->db->query("SELECT * FROM `" . DB_PREFIX . "language` WHERE language_id IN (SELECT `language_id` FROM `" . DB_PREFIX . "user` WHERE `user_id` = '" . (int)$data['user_id'] . "');");
            $langrow = $langquery->row;
            $language = new Language($langrow['directory']);
...
// End Load Vendor Language File
			...
			$message .= $language->get('text_login') . "\n";
	...
	public function getVendorProfile($user_id) {
       // $sql = "SELECT  * FROM `". DB_PREFIX. "vendors` v JOIN " . DB_PREFIX . "user u ON u.user_id = v.user_id ";
	...
	public function getTotalRatingsVendorsByVendorId($vendors) {
      	$query = $this->db->query("SELECT COUNT(*) as total_ratings FROM  " . DB_PREFIX . "rating WHERE ((rating =1 AND STATUS =1 ) OR (rating = 2 AND STATUS =1)) AND vendor_id = '". (int)$vendors  ."'");
		...
		$rating = '<img style="margin-right:10px; width:10%;" src="../image/thropy.png"><span style="font-size:14px;">'.$total.'/'.$total_ratings.' ('.$percentage.'%) </span>';
		return $rating;
	}
