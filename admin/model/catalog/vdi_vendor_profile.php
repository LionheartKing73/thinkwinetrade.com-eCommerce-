...
class ModelCatalogVDIVendorProfile extends Model {
	...
	public function editVendorProfile($user_id, $data) {
		$sql = "UPDATE " . DB_PREFIX . "vendors SET firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "',
			telephone = '" . $this->db->escape($data['telephone']) . "', fax = '" . $this->db->escape($data['fax']) . "', email = '" . $this->db->escape($data['email']) . "',
			paypal_email = '" . $this->db->escape($data['paypal_email']) . "', company_id = '" . $this->db->escape($data['company_id']) . "', iban = '" . $this->db->escape($data['iban']) . "',
			bank_name = '" . $this->db->escape($data['bank_name']) . "', bank_address = '" . $this->db->escape($data['bank_address']) . "', swift_bic = '" . $this->db->escape($data['swift_bic']) . "',
      tax_id = '" . $this->db->escape($data['tax_id']) . "',";
		...
		$sql = "UPDATE " . DB_PREFIX . "vendors SET payment = '" . $this->db->escape($data['payment']) . "' WHERE user_id = '" . (int)$user_id . "'";
		$query = $this->db->query($sql);
		...
		$sql = "UPDATE " . DB_PREFIX . "user SET language_id = '" . (int)$data['language_id'] . "' WHERE user_id = '" . (int)$user_id . "'";
		$query = $this->db->query($sql);
	...
	public function getVendorProfile($user_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "vendors v LEFT JOIN " . DB_PREFIX . "user u USING (user_id) WHERE v.user_id = '" . (int)$user_id . "'");
      	return $query->row;
	}
}
?>
