...
class ModelReportVDISale extends Model {
	...
	//GV
	public function getTotalSalesDashboard($data = array()) {
		
	/*	$sql = "SELECT o.order_id, op.product_id, op.price, op.quantity, p.pf, p.fob_price FROM " . DB_PREFIX . "order o ";
		...
		$sql .= " WHERE o.order_status_id > '20' AND op.vendor_id = '". (int)$this->user->getVP() . "' ";*/
        
        $sql = "SELECT o.order_id, op.product_id, op.price, op.quantity, op.fob_bottle, op.fob_case FROM " . DB_PREFIX . "order o ";
		$sql .= " LEFT JOIN " . DB_PREFIX . "order_product op ON op.order_id = o.order_id ";
		$sql .= " WHERE o.order_status_id > '20' AND op.vendor_id = '". (int)$this->user->getVP() . "' ";
		...
		if ($query->rows) {
			$total = 0;
			foreach ($query->rows as $result) {
				$total = $total + ( $result['fob_case'] * $result['fob_bottle'] ) * $result['quantity'] ;
			}
			return $total;
		} else {
			return false;
