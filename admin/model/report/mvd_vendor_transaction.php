<?php
class ModelReportMVDVendorTransaction extends Model {
	public function getVendorOrders($data = array()) {
		$sql = "SELECT op.product_id AS product_id, o.date_added AS date, o.order_id AS order_id, op.order_status_id AS order_status, op.name AS product_name, op.price AS price,op.quantity AS quantity, op.commission AS commission,op.vendor_total AS amount, op.order_product_id as order_product_id, op.total AS total, op.vendor_id as vendor_id, op.vendor_paid_status AS paid_status,op.tax as tax, (op.tax*op.quantity) as total_tax, op.store_tax as store_tax, op.vendor_tax as vendor_tax,op.title as title,  fob_case, fob_bottle, fob_bottle_full_price FROM `" . DB_PREFIX . "order` o LEFT JOIN `" . DB_PREFIX . "order_product` op ON (o.order_id = op.order_id)";
    ...
    public function getPurchasedFobPrice($product_id, $order_id) {
		$query = $this->db->query("SELECT fob_case, fob_bottle, fob_bottle_full_price FROM " . DB_PREFIX . "order_product WHERE product_id = '" . (int)$product_id . "' and order_id=".$order_id );
		return $query->row;
	}


	public function getVendorCommission( $vendor_id ) {
		
		$sql = "SELECT c.`commission` FROM oc_vendors v LEFT JOIN `oc_commission` c ON c.`commission_id` = v.commission_id ";
		$sql .= " WHERE v.vendor_id = " . (int) $vendor_id;
