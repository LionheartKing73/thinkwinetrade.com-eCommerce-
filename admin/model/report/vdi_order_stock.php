...
class ModelReportVDIOrderStock extends Model {
    ...
    public function getOrderStatus($order_id){
            $query = $this->db->query("SELECT `name` from `" . DB_PREFIX . "order` o left join `" . DB_PREFIX . "order_status` os on o.order_status_id=os.order_status_id
             where o.`order_id` =".$order_id." and os.`language_id` = 2");
             $output_array = array();
             preg_match("/(\d+)/", $query->row['name'], $output_array);
