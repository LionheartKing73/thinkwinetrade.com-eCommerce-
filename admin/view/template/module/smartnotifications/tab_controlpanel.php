<div class="tabbable tabs-left" id="popup_tabs">
    ...
    <ul class="nav nav-tabs popup-list">
        <li class="static"><a class="addNewPopUp"><i class="fa fa-plus"></i> Add New Notification</a></li>
        <?php if (isset($moduleData['SmartNotifications'])) { ?>
            <?php foreach ($moduleData['SmartNotifications'] as $popup) { ?>
            <li><a href="#popup_<?php echo $popup['id']; ?>" data-toggle="tab" data-popup-id="<?php echo $popup['id']; ?>"><i class="fa fa-list-alt"></i> Notification <?php echo $popup['id']; ?> <i class="fa fa-minus-circle removePopUp"></i>
...
function addNewPopUp() {
	...
	$.ajax({
		...
		$('.popup-settings').append(settings_html);
			...
			tpl 	+= '<i class="fa fa-list-alt"></i> Notification '+ count;
...
function selectorsForPopups() {
	...
	$('.daysOfWeekSelect').each(function(e){ 
		if($(this).val() == 0) {
			$(this).parents('.popups').find('.daysOfWeek').hide();
		}
		else {
			$(this).parents('.popups').find('.daysOfWeek').show();
		}
	});

	$('.daysOfWeekSelect').on('change', function(e){ 
		if($(this).val() == 0) {
			$(this).parents('.popups').find('.daysOfWeek').hide(200);
		}
		else {
			$(this).parents('.popups').find('.daysOfWeek').show(200);
		}
	});
