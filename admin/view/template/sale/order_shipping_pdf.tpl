...
/*foreach ($orders as $order) { 
	$title2 = '<div class="footer">ORDER #'.$order['order_id'].' VENDOR ID: '.$order['vendor_id'].' Page: <span class="pagenum"></span></div>';
}
$html .= $title2;*/
  foreach ($orders as $order) { 
	$title_header[] = "PACKING LIST - ORDER #".$order['order_id']."  VENDOR ID: ".$order['vendor_id'];
	//$html .= '<div class="footer">ORDER #'.$order['order_id'].' VENDOR ID: '.$order['vendor_id'].'Page: <span class="pagenum"></span></div>';
	
	$html .= '<div>'.$numb.
	//'<div style="position: fixed; top:0; margin-bottom:50px;">PACKING LIST - ORDER #'.$order['order_id'].'  VENDOR ID: '.$order['vendor_id'].'</div>'.
	'<div  class="table-bordered5a" style="max-width: 100%; padding:1%; page-break-after: always;">'.
    '<table border="0" width="100%"  >'.
		'<tr>'.
		   '<td width="70%" align="left">'.
				'<h1 style="font-size: 30px;">'.strtoupper($text_picklist).' #'.$order['order_id'].'&nbsp;&nbsp;Vendor ID: '.$order['vendor_id'].'</h1>'.
		  '</td>'.
    ...
    '</div>';
	$title_header[] = "PACKING LIST - ORDER #".$order['order_id']."  VENDOR ID: ".$order['vendor_id'];
    $html .= '<div class="table-bordered5a" style="max-width: 100%; padding:1%; page-break-after: always;">'.
	'<h1>'.$text_wh_info.'</h1>'.
    '<br/>'.
    ...
    $title_header[] = "PACKING LIST - ORDER #".$order['order_id']."  VENDOR ID: ".$order['vendor_id'];
	$html .= '<div  class="table-bordered5a" style="width: 100%; page-break-after: always;">'.
	//'<div style="position: fixed; top:0; margin-bottom:50px;">PACKING LIST - ORDER #'.$order['order_id'].'  VENDOR ID: '.$order['vendor_id'].'</div>'.
...
//$numb = $pdf->get_page_number();
//$canvas = $dompdf->get_canvas();
//$font = Font_Metrics::get_font("helvetica", "bold");
//$canvas->page_text(72, 18, "PACKING LIST - ORDER #{PAGE_NUM}  VENDOR ID: {PAGE_COUNT} ", $font, 6, array(0,0,0));
/*$canvas = $dompdf->get_canvas();
$canvas->page_script('
  if ($PAGE_NUM > 1) {
    $font = Font_Metrics::get_font("helvetica", "bold");
    ...
    $pdf->text(72, 18, "Page: $current_page of $total_pages", $font, 6, array(0,0,0));
  }
');*/

//$canvas->page_text(72, 18, "PACKING LIST - ORDER #".$order['order_id']."  VENDOR ID: ".$order['vendor_id']." Page: {PAGE_NUM} of {PAGE_COUNT}", $font, 6, array(0,0,0));
