...
$data['text_login_heading'] = $this->language->get('text_login_heading');
        ...
        $data['back_home'] = $this->language->get('back_home');
        $data['homepage'] = $this->url->link('common/home');

        //langs
        $data['welcome_vendor']= $this->language->get('welcome_vendor');
        $this->load->model('localisation/language');
        $data['_langs'] = $this->model_localisation_language->getLanguages();
        $data['config_language_id'] =$config_language_id;
        //echo "config_language_id: ".$data['config_language_id'];
        $data['switch_language'] = 'http' . (empty($_SERVER['HTTPS']) ? '' : 's') . '://' . $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'];

		return $this->load->view('common/header_login.tpl', $data);
	}
}