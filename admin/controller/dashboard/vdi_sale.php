...
class ControllerDashboardVDISale extends Controller {
	public function index() {
		...
		$data['heading_title'] = $this->language->get('heading_new_title');
		...
		$today = $this->model_report_vdi_sale->getTotalSalesDashboard(array('filter_date_added' => date('Y-m-d', strtotime('-1 day'))));

		$yesterday = $this->model_report_vdi_sale->getTotalSalesDashboard(array('filter_date_added' => date('Y-m-d', strtotime('-2 day'))));
		...
		$sale_total = $this->model_report_vdi_sale->getTotalSalesDashboard();
		
        $data['sale_total'] = $sale_total;//Th-Add;
