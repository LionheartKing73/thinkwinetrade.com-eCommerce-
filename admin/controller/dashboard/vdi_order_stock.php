...
class ControllerDashboardVDIOrderStock extends Controller {
	public function index() {
		...
		foreach ($results as $result) {
            $order_status = $this->model_report_vdi_order_stock->getOrderStatus($result['order_id']);
            ...
            if ((int)$order_status > 2){ 
                ...
                $replace = array(
                    $this->url->link('catalog/vdi_product/edit', 'token=' . $this->session->data['token'] . '&product_id=', 'SSL'),
                    $this->url->link('common/vdi_dashboard', 'token=' . $this->session->data['token'] . '&order_id=', 'SSL')
                );
