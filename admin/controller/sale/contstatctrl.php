...
// End Load Vendor Language File
    	$language->load('mail/email_notification');
		$language->load('sale/contstatctrl');
                                
				$subject = sprintf($language->get('text_subject_expire'),$this->request->post['remaining_days' . "$signup_fee_id"]);
					
				$text = sprintf($language->get('text_to'), $vendor_data['firstname'] . ' ' . $vendor_data['lastname']) . "<br><br>";				
				$text .= sprintf($language->get('text_message_expire'), $this->request->post['remaining_days' . "$signup_fee_id"]) . "<br><br>";
				$text .= $language->get('text_thanks') . "<br>";
				$text .= $this->config->get('config_name') . "<br><br>";
				$text .= $language->get('text_system');
