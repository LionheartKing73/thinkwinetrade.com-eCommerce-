...
class ControllerSaleVDIOrder extends Controller {
	...
	protected function getList() {
        ...
        if (isset($this->request->get['page_f'])) {
			$page_f = $this->request->get['page_f'];
		} else {
			$page_f = 1;
		}
        ...
        if (isset($this->request->get['page_f'])) {
			$url .= '&page_f=' . $this->request->get['page_f'];
		}
		...
		$data['orders'] = array();
         ...
         $data['has_status_filter'] = true;
         //there is just a normal listing
        if ($filter_order_status === null){
            $order_statuses = $this->model_sale_vdi_order->getOrderStatuses();
             $filter_order_status = implode(',',$order_statuses['unfinished']);
             $data['has_status_filter'] = false;
        }
		...
		$results = $this->model_sale_vdi_order->getOrders($filter_data);
     	...
     	foreach ($results as $result) {
			$vendor_products = $this->model_sale_vdi_order->getVendorProductsList($result['order_id']);

			//GV
			$order_fob_total = $this->model_sale_vdi_order->getVendorFobTotal($result['order_id']);
			$result['total'] = $order_fob_total;
			...
			$data['orders'][] = array(
				...
				'new_order'		  => $this->model_sale_vdi_order->isVendorOrderNew($result['order_id']),
				'products_label' => $this->model_sale_vdi_order->getTotalVendorProductsNew($filter = array("filter_order_id" => $result['order_id'])),
				'history_label' => $this->model_sale_vdi_order->getTotalVendorOrdersNew($filter = array("filter_order_id" => $result['order_id'])),
				'total'         => $this->currency->format($result['total'] + $result['shipping_cost'] - $result['coupon_amount'] + ($this->config->get('tax_status') ? ($result['tax'] + $result['shipping_tax'] - $result['coupon_tax'] ) : 0), 'EUR', 1.0),
				'date_added'    => date($this->language->get('date_format_short'), strtotime($result['date_added'])),
				'date_modified' => date($this->language->get('date_format_short'), strtotime($result['date_modified'])),
				'shipping_code' => $result['shipping_code'],
				//'print'         => $this->url->link('sale/vdi_order/sendPO', 'token=' . $this->session->data['token'] . '&order_id=' . $result['order_id'] . $url, 'SSL'),
				'view'          => $this->url->link('sale/vdi_order/info', 'token=' . $this->session->data['token'] . '&order_id=' . $result['order_id'] . $url, 'SSL')
			);
		}

        if (!$data['has_status_filter']){
                $filter_order_status = implode(',',$order_statuses['finished']);
                $filter_data = array(
                    'filter_order_id'      => $filter_order_id,
                    'filter_customer'	   => $filter_customer,
                    'filter_order_status'  => $filter_order_status,
                    ...
                    'sort'                 => $sort,
                    'order'                => $order,
                    'start'                => ($page_f - 1) * $this->config->get('config_limit_admin'),
                    'limit'                => $this->config->get('config_limit_admin')
                );

                $order_total_f = $this->model_sale_vdi_order->getTotalOrders($filter_data);
        ...
        $data['tooltip_no_action'] = $this->language->get('tooltip_no_action');
      	$data['tooltip_products'] = $this->language->get('tooltip_products');
        $data['tooltip_truck'] = $this->language->get('tooltip_truck');

        $data['tab_unfinished_orders'] = $this->language->get('unfinished_orders');
        $data['tab_finished_orders'] = $this->language->get('finished_orders');
		...
		$data['active_tab']= $this->request->get['active_tab'];

		$pagination = new Pagination();
		$pagination->total = $order_total;
		$pagination->page = $page;
		$pagination->limit =  $this->config->get('config_limit_admin');
		$pagination->url = $this->url->link('sale/vdi_order', 'token=' . $this->session->data['token'] . $url . '&active_tab=unfinished&page={page}&page_f='.$page_f, 'SSL');
        ...
        if (!$data['has_status_filter']){
            ...
            $pagination_f = new Pagination();
		    $pagination_f->total = $order_total_f;
		    $pagination_f->page = $page_f;
		    $pagination_f->limit =  $this->config->get('config_limit_admin');
     	    $pagination_f->url = $this->url->link('sale/vdi_order', 'token=' . $this->session->data['token'] . $url . '&active_tab=finished&page_f={page}&page='.$page, 'SSL');

		    $data['pagination_f'] = $pagination_f->render();

            $data['results_f'] = sprintf($this->language->get('text_pagination'), ($order_total_f) ? (($page_f - 1) * $this->config->get('config_limit_admin')) + 1 : 0, ((($page_f - 1) * $this->config->get('config_limit_admin')) > ($order_total_f - $this->config->get('config_limit_admin'))) ? $order_total_f : ((($page_f - 1) * $this->config->get('config_limit_admin')) + $this->config->get('config_limit_admin')), $order_total_f, ceil($order_total_f / $this->config->get('config_limit_admin')));
	...
	public function info() {
		...
		if ($order_info) {
			...
			if(!$this->model_sale_vdi_order->isVendorOrderNew($order_id)) {
				$this->model_sale_vdi_order->setVendorOrderSeen($order_id);
			}
			...
			$data['text_choose_shipping'] = $this->language->get('text_choose_shipping');
			...
			$data['button_invoice_print'] = $this->language->get('button_invoice_print');
			$data['button_shipping_print'] = $this->language->get('button_shipping_print');
			$data['button_packing_list_print'] = $this->language->get('button_packing_list_print');
			$data['button_edit'] = $this->language->get('button_edit');
			...
			$data['text_confirmed_yes'] = $this->language->get('text_confirmed_yes');
			$data['text_confirmed_no'] = $this->language->get('text_confirmed_no');
			$data['text_products_confirmed'] = $this->language->get('text_products_confirmed');
			$data['text_products_need_confirmation'] = $this->language->get('text_products_need_confirmation');
			$data['text_history_need_confirmation'] = $this->language->get('text_history_need_confirmation');
			$data['text_history_ready_to_confirm'] = $this->language->get('text_history_ready_to_confirm');
			$data['text_history_tracking_information'] = $this->language->get('text_history_tracking_information');
			$data['text_field_transporter'] = $this->language->get('text_field_transporter');
			$data['text_field_ref_number'] = $this->language->get('text_field_ref_number');
			$data['column_confirmed'] = $this->language->get('column_confirmed');
			$data['tooltip_products'] = $this->language->get('tooltip_products');
			$data['tooltip_history'] = $this->language->get('tooltip_history');
			...
			$data['shipping_pdf'] = $this->url->link('sale/vdi_order/shipping_pdf', 'token=' . $this->session->data['token'] . '&order_id=' . (int)$this->request->get['order_id'], 'SSL');
			...
			foreach ($products as $product) {
				...
				// BUG
				$fob = $this->model_sale_vdi_order->getFobPrice2( $product['product_id'], $this->request->get['order_id'] );
				$product['price'] = $fob['fob_price'] * $fob['pf'];
				$product['total'] = $product['quantity'] * $product['price'];
			    $fob_total += $product['total'];


				$price ='';
				$total = '';
				if (floatval($fob['fob_price']) == floatval($fob['fob_bottle_full_price']) ||   floatval($fob['fob_bottle_full_price'])==0){

				     $price = $this->currency->format($product['price'] + ($this->config->get('config_tax') ? $product['tax'] : 0), $order_info['currency_code'], $order_info['currency_value']);
					 $total = $this->currency->format($product['total'], $order_info['currency_code'], $order_info['currency_value']);
				}
				else{
				    $price = '<del>'.$this->currency->format(($fob['fob_bottle_full_price'] * $fob['pf']) + ($this->config->get('config_tax') ? $product['tax'] : 0), $order_info['currency_code'], $order_info['currency_value']).'</del>
                             <br />'.
                             ((strlen($product['obs'])>1)?"<font color='#cc0000' size='-1'>Discount: </font> ":"<font color='#cc0000' size='-1'>Promo: </font> ")
                             .'<font color="#cc0000">'.$this->currency->format(($fob['fob_price'] * $fob['pf']) + ($this->config->get('config_tax') ? $product['tax'] : 0), $order_info['currency_code'], $order_info['currency_value']).'</font>';

					$total = '<del>'.$this->currency->format($product['quantity'] * $fob['fob_bottle_full_price'] * $fob['pf'] + ($this->config->get('config_tax') ? ($product['tax'] * $product['quantity']) : 0), $order_info['currency_code'], $order_info['currency_value']).'</del><br /><font color="#cc0000">'.$this->currency->format($product['quantity'] * $fob['fob_price'] * $fob['pf'] + ($this->config->get('config_tax') ? ($product['tax'] * $product['quantity']) : 0), $order_info['currency_code'], $order_info['currency_value']).'</font>';
				...
				$data['products'][] = array(
					...
					'pallet_product_id' => $product['pallet_product_id'],
					...
					'vintage'  => $vintage,
					'price'    		   =>$price,
					'price_eur'    		   => $this->currency->format($product['price'], 'EUR', 1.0),
					'total'    		   =>$total,
					'total_eur'    		   => $this->currency->format($product['total'] , 'EUR', 1.0),
					'confirmed'			 => $product['vendor_confirmed'],
			...
			//$getOrderTotals = $this->model_sale_vdi_order->getOrderTotalsFOB($this->request->get['order_id']);

			foreach ($getOrderTotals as $order_totals) {
				//GV
				$order_totals['total'] = $fob_total;
				$data['subtotal'] = $this->currency->format($order_totals['total']-($order_totals['total']*0.2)/1.2, $order_info['currency_code'], $order_info['currency_value']);
                $data['vat'] = $this->currency->format(($order_totals['total']*0.2)/1.2, $order_info['currency_code'], $order_info['currency_value']);
				$data['subtotal_eur'] = $this->currency->format($order_totals['total'], 'EUR', 1.0);
				...
				$data['totals_eur'] = $this->currency->format($order_totals['total'] + $order_totals['shipping_cost'] - $order_totals['coupon_amount'] + ($this->config->get('tax_status') ? ($order_totals['shipping_tax'] + $order_totals['tax'] - $order_totals['coupon_tax']): 0), 'EUR', 1.0);
			...
			foreach($data['order_statuses'] as $order_status) {
				$data['order_statuses_by_id'][$order_status['order_status_id']] = $order_status;
			}

			$data['order_status_id'] = $order_info['order_status_id'];

			$vendor_statuses = array(18, 2, 19, 3);
                        $data['order_status_id'] = $vendor_order_status_id;
			$data['button_disabled'] = '';
			$data['history_label'] = '';

			$data['status_waiting_po_confirmation'] = $this->config->get('po_status_waiting_po_confirmation_id');
			$data['status_po_confirmed'] = $this->config->get('po_status_po_confirmed_id');
			$data['status_shipping_confirmed_id'] = $this->config->get('po_status_shipping_confirmed_id');

			if(in_array($data['order_status_id'], $vendor_statuses)) {
				if($data['order_status_id'] == $this->config->get('po_status_waiting_po_confirmation_id')) {
					$data['button_disabled'] = $this->model_sale_vdi_order->isVendorProductsConfirmed($data['order_id'], $data['vendor_id']);
					$data['button_action'] = $this->language->get('button_confirm_po');
					if(!$data['button_disabled']) { $data['history_label'] = 1; }
				} elseif ($data['order_status_id'] == $this->config->get('po_status_po_confirmed_id')) {
					$data['button_disabled'] = $this->model_sale_vdi_order->isAllVendorsConfirmed($data['order_id'], $this->config->get('po_status_waiting_po_confirmation_id'));
					$data['button_action'] = $this->language->get('button_confirm_shipping');
					$data['text_noactions'] = $this->language->get('text_wait_po_confirm');
					$this->load->model('module/vdi_my_carriers');
					$data['transporters'] = $this->model_module_vdi_my_carriers->getCarriersDropdown();
					$data['shipping_by_myself'] = $this->language->get('shipping_by_myself');
					if(!$data['button_disabled']) { $data['history_label'] = 1; }
				} else {
					$data['text_noactions'] = $this->language->get('text_noactions');
				}
			} else {
				$data['order_status_id'] = 5;
				$data['text_noactions'] = $this->language->get('text_noactions');
			}
			$data['total'] = $this->currency->format($fob_total, $order_info['currency_code'], $order_info['currency_value']);

			$data['products_label'] = $this->model_sale_vdi_order->getTotalVendorProductsNew($filter = array("filter_order_id" => $data['order_id']));
	...
	public function invoice() {
		...
		foreach ($orders as $order_id) {
			...
			if ($order_info) {
				...
				if ($store_info) {
					$site_comment = $store_info['config_comment'];
				} else {
					$site_comment = $this->config->get('config_comment');
				}
			...
			if ($order_info && $order_info['shipping_code']) {
				...
				foreach ($products as $product) {
					...
					$product_data[] = array(
						...
						'location1' => $product_info['location'],
						...
						//'weight'   => $this->weight->format( ($product_info['weight'] * $product['quantity']), $this->config->get('config_weight_class_id'), $this->language->get('decimal_point'), $this->language->get('thousand_point'))
					);
				}
				...
				//GV
				$data['vendor_id'] = $this->user->getVP();
				$data['admin_comment'] =  nl2br( $this->model_sale_vdi_order->getComment( $order_id ) );
				...
				$data['wh_total_quantity'] = $wh_total_quantity;
				$data['wh_total_weight'] = $this->weight->format( $wh_total_weight , $this->config->get('config_weight_class_id'), $this->language->get('decimal_point'), $this->language->get('thousand_point'));
				$data['uom'] = $this->model_sale_vdi_order->getUOM( $this->config->get('config_weight_class_id') );
				$data['total_pallet'] = $this->model_sale_vdi_order->getOrderPalletTotal( $order_id );
				$data['total_product'] = $this->model_sale_vdi_order->getOrderProductTotal( $order_id );
				include 'controller/bar/switch.php';
				//$data['column_location1'] = $data['column_location'];
	...
	public function confirmProduct() {
		...
		if (isset($this->request->get['order_id']) && isset($this->request->get['product_id'])) {
			$this->model_sale_vdi_order->confirmProduct($this->request->get['order_id'], $this->request->get['product_id']);
			$json['success'] = "OK.";
		} else {
        ...
        public function ajax_popup(){
            $this->load->language('sale/popup');
            $data['popups'] = array(
                array(
                    'title' => $this->language->get('confirmation_title'),
                    'text' => $this->language->get('confirmation_tip'),
                    'element' => "#tab-product thead td:nth-child(6)",
                    'tab'   => "#tab-product",
                    'req'   => "#tab-product label.label-danger"
                ),
                array(
                    'title' => $this->language->get('confirm_button_title'),
                    'text' => $this->language->get('confirm_button_tip'),
                    'element' => "#tab-product .btn.btn-success:first",
                    'req'   => "#tab-product label.label-danger"
                ),
                array(
                    'title' => $this->language->get('confirm_order_title'),
                    'text' => $this->language->get('confirm_order_tip'),
                    'element' => "#button-history",
                    'req' => "#button-history:contains('Confirmer ce Bon de Commande (PO)')",
                    'tab' => "#tab-history"
                ),
                array(
                    'title' => $this->language->get('shipping_title'),
                    'text' => $this->language->get('shipping_tip'),
                    'element' => "#po_transporter",
                    'req' => "#po_transporter",
                    'tab' => "#tab-history"
                ),
                array(
                    'title' => $this->language->get('shipping_button_title'),
                    'text' => $this->language->get('shipping_button_tip'),
                    'element' => "#button-history",
                    'req' => "#button-history:contains('Transporteur')",
                    'tab' => "#tab-history"
                ),
                array(
                    'title' => $this->language->get('print_title'),
                    'text' => $this->language->get('print_tip'),
                    'element' => "#tab-history a.btn-danger",
                    'req' => "#tab-history a.btn-danger",
                    'tab' => "#tab-history"
                )
            );
