<?php
class ControllerReportVDITransaction extends Controller { 
	public function index() {  
		$this->load->controller('common/vdi_dashboard/checklist_venue');
		
		$this->load->language('report/vdi_transaction');

		$this->document->setTitle($this->language->get('heading_title'));
		
		$this->load->model('report/vdi_transaction');
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('report/vdi_transaction', 'token=' . $this->session->data['token'] . $url, 'SSL')
		);
		...
		$order_total = $this->model_report_vdi_transaction->getVendorTotalOrders($filter_data);
		//GV
		$this->load->model('report/vdi_sale');
		$net_sales = $this->model_report_vdi_sale->getTotalSalesDashboard();
		$data['net_sales'] = $this->currency->format( $net_sales, $this->config->get('config_currency'));
		
		//$results = $this->model_report_vdi_transaction->getVendorOrders($filter_data);
		$results = $this->model_report_vdi_transaction->getVendorOrdersFiltered($filter_data);
		$store_revenues = $this->model_report_vdi_transaction->getVendorTotalAmount($filter_data);
		$data['vendors_name'] = $this->model_report_vdi_transaction->getVendorsName($filter_data);
		$data['login_vendor_id'] = $this->user->getVP();
		
		$payment_histories = $this->model_report_vdi_transaction->getPaymentHistory(0);
		
		$shipping_charges = $this->model_report_vdi_transaction->getShippingChargedTotal($filter_data);
		...
		$discount_coupons = $this->model_report_vdi_transaction->getCouponTotal($filter_data);
		$coupon_amount = 0;
		...
		$data['commission_data'] = $this->model_report_vdi_transaction->getCommissionData();
		...
		foreach ($results as $result) {
			//GV
			$fob = array();
			$fob = $this->model_report_vdi_transaction->getFobPrice( $result['product_id'] );
			$result['price'] = $fob['pf'] * $fob['fob_price'];
			$result['total'] = $result['price'] * $result['quantity'];
			$commisson = $this->model_report_vdi_transaction->getVendorCommission($result['vendor_id']);
			//$result['commission'] = $result['total'] / 100 * $commisson;
			
			$result['commission'] = 0.0;
			$result['amount'] = $result['total'] - $result['commission'];
			...
			$data['orders'][] = array(
				...
				'product_name'  => $result['product_name'] . '<br/><b>(' . $this->model_report_vdi_transaction->getVendorName($result['vendor_id']) . ')</b>',
				...
				'paypal_email'	=> $this->model_report_vdi_transaction->getVendorPaypalEmail($result['vendor_id']),
		...
		$data['title_gross_revenue'] = $this->language->get('title_gross_revenue');
		
		//GV
		$data['text_net_sales'] = $this->language->get('text_net_sales');
		...
		$data['addPayment'] = $this->url->link('report/vdi_transaction/insert', 'token=' . $this->session->data['token'] . $url, 'SSL');
		
		$pagination = new Pagination();
		$pagination->total = $order_total;
		$pagination->page = $page;
		$pagination->limit = $this->config->get('config_limit_admin');
		$pagination->url = $this->url->link('report/vdi_transaction', 'token=' . $this->session->data['token'] . $url . '&page={page}', 'SSL');
		...
		$this->response->setOutput($this->load->view('report/vdi_transaction.tpl', $data));
	}
}
?>