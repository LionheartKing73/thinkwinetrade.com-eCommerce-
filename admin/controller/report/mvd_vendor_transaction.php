...
class ControllerReportMVDVendorTransaction extends Controller { 
	public function index() {  
		...
		foreach ($results as $result) {
			...
			//GV
		/*	$fob = array();
			$fob = $this->model_report_mvd_vendor_transaction->getFobPrice( $result['product_id'] );
				echo '<pre>';
			var_dump($fob);
			echo '</pre>';
			$result['vendor_price'] = (float) $fob['pf'] * (float) $fob['fob_price'];
			$profit = $result['total'] - ( $result['vendor_price'] * $result['quantity'] );
			$result['total'] = $result['vendor_price'] * $result['quantity'];
			$amount = $result['total'] + $profit;*/
			
			$result['vendor_price'] = (float) $result['fob_case'] * (float) $result['fob_bottle'];
			$vendor_price = $this->currency->format($result['vendor_price'] + ($this->config->get('tax_status') ? $result['tax']: 0), $this->config->get('config_currency'));
			
			if ((float)$result['fob_bottle'] != (float)$result['fob_bottle_full_price']){
				$vendor_price = '<del>'.$this->currency->format((float) $result['fob_case'] * (float) $result['fob_bottle_full_price'] + ($this->config->get('tax_status') ? $result['tax']: 0), $this->config->get('config_currency')).'</del><br /><font color="#cc0000">'.$vendor_price.'</font>';
         ...
         /*   if ($result['order_id'] == 5){
			...
			$data['orders'][] = array(
				...
				'title'			=> $result['title'],
				'commission'  	=> $this->currency->format( $profit + ($this->config->get('tax_status') ? $result['store_tax']: 0), $this->config->get('config_currency')),
				...
				'amount'  		=> $this->currency->format($amount + ($this->config->get('tax_status') ? $result['vendor_tax']: 0),$this->config->get('config_currency')),
				'amountHKD'  		=> $this->currency->format($amount + ($this->config->get('tax_status') ? $result['vendor_tax']: 0), 'HKD' ),
				//'amount'  		=> $this->currency->format($result['amount'] + ($this->config->get('tax_status') ? $result['vendor_tax']: 0),$this->config->get('config_currency')),
				'paypal_amount'	=> $this->currency->format($result['total'] + ($this->config->get('tax_status') ? $result['vendor_tax']: 0),$this->config->get('config_currency'), false, false),
		...
		//GV
		$data['column_vunit_price'] = $this->language->get('column_vunit_price');
	...
	public function addPaymentRecord() {
		...
		if ($getOPs->row) {
			$order_detail[] = array(
                ...
                'fob_bottle' => $getOPs->row['fob_bottle'],
                'fob_bottle_full_price' => $getOPs->row['fob_bottle_full_price'],
                'fob_case' => $getOPs->row['fob_case']
			);
          
			//GV
	
			$unit_price = $getOPs->row['fob_case'] * $getOPs->row['fob_bottle'] ;
			$vendor_total = $getOPs->row['quantity'] * $unit_price;
            
            
			$this->db->query("INSERT INTO " . DB_PREFIX . "vendor_payment SET vendor_id = '" . (int)$getOPs->row['vendor_id'] . "', payment_info = '" . serialize($order_detail) . "', payment_amount = '" . (float)($vendor_total +($this->config->get('tax_status') ? $getOPs->row['vendor_tax']: 0)) . "', payment_type = '" . $this->language->get('text_paypal_standard') ."', payment_status = '5', payment_date = Now()");
...
//            $language->load('mail/vendor_email');
			...
			//$this->language->load('mail/vendor_email');
			

			//$this->load->language('mail/vendor_email');
			$this->load->model('sale/vdi_order');
			$order_info = $this->model_sale_vdi_order->getOrder($order_id);
			
			$data = array();

          $data['logo'] = HTTP_CATALOG . 'image/' . 'logo_new.png';
            //$data['store_name'] = $order_info['store_name'];
            //$data['store_url'] = $order_info['store_url'];
			$data['order_id'] = $order_id;
			$data['amount'] = $this->currency->format( $vendor_total, $order_info['currency_code'], $order_info['currency_value']);

            /*show vendor name*/
            $vendor = $this->db->query("SELECT *, CONCAT(firstname,' ',lastname) AS contact_name, email FROM `" . DB_PREFIX . "vendors` WHERE vendor_id = '" . (int) $getOPs->row['vendor_id'] . "'");
// Load Vendor Language File MH 2017-02-07
            $langquery = $this->db->query("SELECT * FROM `" . DB_PREFIX . "language` WHERE language_id IN (SELECT `language_id` FROM `" . DB_PREFIX . "user` WHERE `user_id` = '" . (int)$vendor->row['user_id'] . "');");
            $langrow = $langquery->row;
            $language = new Language($langrow['directory']);

            if (version_compare(VERSION, '2.0.2', '>=')) {
                $language->load($langrow['directory']);
            } else if (version_compare(VERSION, '2.0.1', '<')) {
                $language->load($langrow['filename']);
            } else {
                $language->load('default');
            }
// End Load Vendor Language File
            $language->load('mail/vendor_email');
            
            $data['vendor_name'] = '<b>' . $language->get('text_title') . $vendor->row['contact_name'] . '</b>,' . "\n\n";

			$data['column_payment_amount'] = $language->get('column_payment_amount');
			$data['column_payment_type'] = $language->get('column_payment_type');
			$data['column_payment_date'] = $language->get('column_payment_date');
			$data['column_order_product'] = $language->get('column_order_product');
			$data['config_currency'] = $this->config->get('config_currency');
            
			$payment_histories = $this->db->query( "SELECT * FROM `" . DB_PREFIX ."vendor_payment` WHERE vendor_id = '" . (int)$getOPs->row['vendor_id'] ."' ORDER BY payment_date DESC LIMIT 1 ");
			
			foreach ($payment_histories->rows as $payment_history) {
				$data['histories'][] = array (
					'payment_id'	=> $payment_history['payment_id'],
					'details'		=> unserialize(trim($payment_history['payment_info'])),
					'amount'		=> $this->currency->format($payment_history['payment_amount'], $this->config->get('config_currency')),
					'payment_type'	=> $payment_history['payment_type'],
					'date'			=> date($language->get('date_format_short2'), strtotime($payment_history['payment_date']))				
					);
			}

			$vendor_email = $vendor->row['email'];
			$data['title'] = $language->get('text_txn_subject');
			$data['subject'] = $language->get('text_txn_subject');
			$subject = $data['subject'];
			$data['text_dear'] = $language->get('text_dear');
			$data['text_txn_body'] = sprintf($language->get('text_txn_body'), $order_id, $data['amount']);
			$data['text_txn_body1'] = $language->get('text_txn_body1');
			//'Thinwinetrade.com just sent you a payment, thank you for you cooperation';
			
            $html = $this->load->view('mail/vendor_payment_email.tpl', $data);
