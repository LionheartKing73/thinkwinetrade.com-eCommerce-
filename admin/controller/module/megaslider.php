...
class ControllerModuleMegaSlider extends Controller {
	...
	public function index() {   
		...
		$language = $this->load->language('module/megaslider');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->response->redirect($this->url->link('module/megaslider&submit=true', 'token=' . $this->session->data['token'], 'SSL'));
		...
		$data['breadcrumbs'][] = array(
			...
			'href'      => $this->url->link('module/megaslider', 'token=' . $this->session->data['token'], 'SSL'),
			'separator' => ' :: '
		);
		
		$data['action'] = $this->url->link('module/megaslider', 'token=' . $this->session->data['token'], 'SSL');
		...
		for ($language_id = 1; $language_id <= $language_total; $language_id++) {
		...
		//Send the output.
		$this->response->setOutput($this->load->view('module/megaslider.tpl', $data));
	}
	...
	private function validate() {
		if (!$this->user->hasPermission('modify', 'module/megaslider')) {
