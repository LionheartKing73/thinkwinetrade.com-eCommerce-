...
class ControllerCatalogMVDProduct extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            if (isset($this->request->get['filter_status_vendor'])) {
				$url .= '&filter_status_vendor=' . $this->request->get['filter_status_vendor'];
			}
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            $this->model_catalog_mvd_product->editProduct($this->request->get['product_id'], $this->request->post);
               ...
               $this->model_catalog_skuautogen->updateSKUProduct($this->request->get['product_id']);
			...
			//mvds
			 if ($this->config->get('mvd_product_edit_approval')) {
                ...
                if ($this->config->get('mvd_product_approval') && $this->config->get('mvd_product_notification')) {
					if (!empty($this->request->post['product_name']) && ($this->request->post['status'] == '1') && ($this->request->post['pending_status'] == '5')) {
						$this->approve_notification($this->request->post['product_name'],$this->request->post['vendor']);
					}
				}
	...
	protected function getList() {
        ...
        if (isset($this->request->get['filter_status_vendor'])) {
			$filter_status_vendor = $this->request->get['filter_status_vendor'];
		} else {
			$filter_status_vendor = null;
		}
		...
		$filter_data = array(
            ...
            'filter_status_vendor'   => $filter_status_vendor,
			'sort'            => $sort,
			'order'           => $order,
		...
		foreach ($results as $result) {
			if (is_file(DIR_IMAGE . $result['pimage'])) {
			...
			$product_specials = $this->model_catalog_mvd_product->getProductSpecials($result['product_id']);
            //$fob_price = $this->currency->format($result['fob_price']*$result['pf']);
			...
			foreach ($product_specials  as $product_special) {
				if (($product_special['date_start'] == '0000-00-00' || strtotime($product_special['date_start']) < time()) && ($product_special['date_end'] == '0000-00-00' || strtotime($product_special['date_end']) > time())) {
                    ...
                    $fob_price = '<del>'.$this->currency->format($fob_price).'</del><br /><font color="#cc0000">'.$this->currency->format($product_special['fob_price']).'</font>';
				}
				break;
			}
...
$data['entry_pf'] = $this->language->get('entry_pf');
    ...
    $data['entry_sp_price'] = $this->language->get('entry_sp_price');
		$data['entry_price'] = $this->language->get('entry_price');
		$data['entry_quantity'] = $this->language->get('entry_quantity');
		$data['entry_status'] = $this->language->get('entry_status');
        $data['entry_status_vendor'] = $this->language->get('entry_status_vendor');
        ...
        $data['filter_status_vendor'] = $filter_status_vendor;
...
$data['entry_pf'] = $this->language->get('entry_pf');
    ...
    $data['entry_sp_price'] = $this->language->get('entry_sp_price');
    	...
    	if (isset($this->request->post['fakeprise'])) {
			$data['fakeprise'] = $this->request->post['fakeprise'];
		} elseif (isset($product_info)) {
			$data['fakeprise'] = $product_info['fakeprice'];
		} else {
			$data['fakeprise'] = 0;
		}
      ...
      $data['price'] = '';
		...
		foreach ($product_specials as $product_special) {
			$data['product_specials'][] = array(
                ...
                'fob_price'         => $product_special['fob_price'],
                'sp_price'          => $product_special['sp_price'],
...
// Load Vendor Language File MH 2017-02-07
            //Store excisting language to reset after
            $keeplang = $this->registry->get('language');
            $langquery = $this->db->query("SELECT * FROM `" . DB_PREFIX . "language` WHERE language_id IN (SELECT `language_id` FROM `" . DB_PREFIX . "user` WHERE `user_id` = '" . (int)$vendor_data['user_id'] . "');");
            $langrow = $langquery->row;
            $lang = new Language($langrow['directory']);

            if (version_compare(VERSION, '2.0.2', '>=')) {
                $lang->load($langrow['directory']);
            } else if (version_compare(VERSION, '2.0.1', '<')) {
                $lang->load($langrow['filename']);
            } else {
                $lang->load('default');
            }
...
// Load Vendor Language File MH 2017-02-07
            //Reset language
            $this->registry->set('language', $keeplang);
// End Load Vendor Language File
	}
	//mvde
