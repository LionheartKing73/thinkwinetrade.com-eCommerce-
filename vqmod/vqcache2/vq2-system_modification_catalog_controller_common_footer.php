...
class ControllerCommonFooter extends Controller {
	...
	public function index() {
		...
		$modules = $this->model_design_layout->getLayoutModules(4, 'content_top');

		foreach ($modules as $module) {
			$part = explode('.', $module['code']);

			if (isset($part[0]) && $this->config->get($part[0] . '_status')) {
				$data['top_blocks'][] = $this->load->controller('module/' . $part[0]);
			}
			...
			if (isset($part[1])) {
				...
				if ($setting_info && $setting_info['status']) {
					$data['top_blocks'][] = $this->load->controller('module/' . $part[0], $setting_info);
				}
			}
		}
    
    	$modules = $this->model_design_layout->getLayoutModules(4, 'content_bottom');
