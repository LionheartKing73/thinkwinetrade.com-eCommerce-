...
class ModelCheckoutOrder extends Model {
	...
	public function addOrder($data) {
		...
		if (isset($data['products'])) {
            ...
            $price = $query_price->row['fob_price'];
            $obs = "0";
            $query_special_price = $this->db->query("SELECT fob_price FROM " . DB_PREFIX . "product_special WHERE product_id = '" . (int)$product['product_id'] . "' and customer_group_id=".(int)$this->config->get('config_customer_group_id') );


            $fob_bottle_full_price = (float)$price;
            if ($query_special_price->num_rows == 1){
                $price = $query_special_price->row['fob_price'];
                $fob_bottle_full_price = (float)$query_price->row['fob_price'];
                $obs="1";
             }
            ...
            $this->load->model('catalog/product');
             $discount = $this->model_catalog_product->getProductDiscount((int)$product['product_id'],
                                                                          (int)$product['quantity']);

             if($discount['price'] !== null) {
                              $fob_bottle_full_price = $price;
                              $price = $discount['fob_price'];
                              $obs = $discount['step-text'];
             }


        	$this->db->query("INSERT INTO " . DB_PREFIX . "order_product SET order_id = '" . (int)$order_id .
                             "', product_id = '" . (int)$product['product_id'] . "', name = '" .
                              $this->db->escape($product['name']) . "', model = '" .
                              $this->db->escape($product['model']) . "', quantity = '" .
                              (int)$product['quantity'] . "', price = '" . (float)$product['price'] .
                              "', total = '" . (float)$product['total'] . "', tax = '" . (float)$product['tax'] .
                              "', vendor_id = '" . (int)$product['vendor_id'] . "', commission =  '" .
                              (float)$product['commission'] . "', title = '" . $this->db->escape($product['title']) .
                               "', vendor_total = '" . (float)$product['vendor_total'] .
                               "', vendor_tax = '" . (float)$product['vendor_tax'] .
                               "', store_tax = '" . (float)$product['store_tax'] .
                               "',reward = '" . (int)$product['reward'] . "', obs='".$obs."', fob_case = '" .
                               (int)$query_price->row['pf'] . "', fob_bottle = '" . (float)$price .
                                "', fob_bottle_full_price = '" .(float)$fob_bottle_full_price. "'");
...
$getVenId = $this->db->query("SELECT vs.vendor_id AS vendor_id FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "vendor v ON (p.product_id = v.vproduct_id) LEFT JOIN " . DB_PREFIX . "vendors vs ON (v.vendor = vs.vendor_id) WHERE p.product_id = '" . (int)$product['product_id'] . "'");
	...
	public function getOrder($order_id) {
		...
		if ($order_info) {
							...
							foreach ($vendor_products as $vendor_product) {
								if ($vendor['vendor_id'] == $vendor_product['vendor_id']) {
									$data['vendor_products'][] = array(
										...
										'price'	   => "price_test".$this->currency->format($vendor_product['price'] + ($this->config->get('tax_status') ? $vendor_product['tax'] : 0)),
			...
			$data['additional_curr'] = isset($_COOKIE["additional_currency"]) ? $_COOKIE["additional_currency"] : "HKD";
                ...
                $sp_price = $product['fob_bottle_full_price'];

                foreach($this->config->get('fob_margins') as $margin) {
                          if($margin['fob'] == 1) {
                                eval('$sp_price = '."$sp_price + ($product[fob_bottle_full_price] $margin[sign] $margin[value])".';');
                              } else {
                                eval('$sp_price = '."$sp_price $margin[sign] $margin[value]".';');
                              }
                 }
                 
                 $sp_price *= $product['fob_case'];
                 $hkd = $this->currency->format($product['price'], 
			$data['additional_curr'],$this->currency->getValue($data['additional_curr'])
			);
                 $total_hkd += $product['price']*$product['quantity']; 
                 
                if (floatval($product['fob_price']) == floatval($product['fob_bottle_full_price'])){
                      ...
                      $hkd = '<del>'.str_replace(",","",$this->currency->format($product['quantity'] * $sp_price +
                                                                  ($this->config->get('config_tax') ? ($product['tax'] * $product['quantity']) : 0),
                                                                   
			$data['additional_curr'],$this->currency->getValue($data['additional_curr'])
			)).'</del><br />';
                      $hkd.= '<span class="discounted_price">'. 
                              str_replace(",","", $this->currency->format($product['quantity']*$product['price'] + 
                                                                ($this->config->get('config_tax') ? ($product['tax'] * $product['quantity']) : 0),
                                                                  
			$data['additional_curr'],$this->currency->getValue($data['additional_curr'])
			)).'</span>';   
			          $price = '<del>'.$this->currency->format($sp_price + ($this->config->get('config_tax') ? $product['tax'] : 0), $order_info['currency_code'], $order_info['currency_value']).'</del>
                             <br />'.
                             ((strlen($product['obs'])>1)? $this->language->get('label_discount'): $this->language->get('label_special'))
                             .' <span class="discounted_price">'.$this->currency->format($product['price'] + ($this->config->get('config_tax') ? $product['tax'] : 0), $order_info['currency_code'], $order_info['currency_value']).'</span>';

					   $total = '<del>'.$this->currency->format($product['quantity'] * $sp_price +
                          ($this->config->get('config_tax') ? ($product['tax'] * $product['quantity']) : 0),
                                                          $order_info['currency_code'], $order_info['currency_value']).
                                     '</del><br /> <span class="discounted_price">'.
                                               $this->currency->format($product['quantity']*$product['price'] + 
                            ($this->config->get('config_tax') ? ($product['tax'] * $product['quantity']) : 0), $order_info['currency_code'], $order_info['currency_value']).'</span>';
			...
			'title' => $data['additional_curr'],
			
						'text'  =>  
			str_replace(",", "",$this->currency->format($hkd_, $data['additional_curr'], $this->currency->getValue($data['additional_curr']))),
		...
		$this->event->trigger('post.order.history.add', $order_id);
				...
				if (!empty($address)) {
					...
					$country_id	= isset($this->session->data['payment_address']['country_id'])? $this->session->data['payment_address']['country_id']:0;
					$zone_id 	= isset($this->session->data['payment_address']['zone_id'])? $this->session->data['payment_address']['zone_id']:0;
				}
