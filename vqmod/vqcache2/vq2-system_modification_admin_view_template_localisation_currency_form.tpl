          ...
          <div class="form-group">
            ...
            <div class="col-sm-10">
              <input type="text" name="value" value="<?php echo $value; ?>" placeholder="<?php echo $entry_value; ?>" id="input-value" class="form-control" />
				...
				<div class="form-group">
            <label class="col-sm-2 control-label" for="input-status">Used in</label>
			<script type="text/javascript">
				$('document').ready(function(){ $(".countryselect").select2(); });
			</script>

            <div class="col-sm-10">
              <select name="countries[]" id="countries" name="countries" multiple="multiple" class="form-control countryselect">
			  <?php foreach($countries as $country) { if($country['status'] != 0) { ?>
                <option value="<?php echo $country['country_id']; ?>" <?php echo isset($assigned_countries) && in_array($country['country_id'], $assigned_countries) ? 'selected' : ''; ?>><?php echo $country['name']; ?></option>
			  <?php }} ?>
              </select>
            </div>
          </div>
