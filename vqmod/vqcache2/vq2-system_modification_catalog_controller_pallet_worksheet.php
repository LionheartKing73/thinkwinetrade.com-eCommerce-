...
public function slider_palletdata()
	{
		$this->load->model('pallet/worksheet');
		$this->load->model('catalog/product');
		$this->response->addHeader('Content-Type: application/json');
		$resultdata =  array('pallets'=>array());
		if(!$this->customer->isLogged()) {
			$this->response->setOutput(json_encode(array('status' => 'ok', 'data' => $resultdata)));
			return;
		} else {
			...
			for($p = 0; $p < count($palletIds); $p++) {
						...
						if($vendorLimit = $this->model_pallet_worksheet->getBoxesPerVendor($pallet_id, $product['vendor'])) {
						 ...
						 $total_quantity_of_this_product  = $this->model_pallet_worksheet->getWorksheetProductQty($worksheet_id, $product['product_id']);
							 ...
							 if($discount['price'] !== null) {
											  ...
											  $product_info['dsc_label'] = $this->language->get('label_discount');
							 }
							...
							if ($product['price'] != $fob_data['price']){
									...
									$price_per_bottle = $this->currency->format($product['price'] / $fob_data['pf']);
							} else {
								$price_per_bottle = $this->currency->format($fob_data['price'] / $fob_data['pf']);
							}


						$pallet['products'][] = array(
							...
							'price'     => $price,
							'total'     => $total,
							'vetdor_limit' => $product['vendor_limit'],
							'href'      => $this->url->link('product/product', 'product_id=' . $product['product_id']),
                        	'attr' =>  $this->model_pallet_worksheet->getProductAttributes($product['product_id'], 1)
						);
					}
            	...
            	$pallet['locked'] = $palletLocked;
				$pallet['progress']['limit'] = $palletSize;
				$pallet['space']['current'] = $currentProductsCount;
				$pallet['progress']['current'] = 100*$currentProductsCount/$palletSize;
				$infos = array();
				if($currentProductsCount < $palletSize) {
					...
					$pallet['infos'][] = $infos;
					$pallet['space']['left'] = $spaceLeft;
					$pallet['progress']['left'] = 100*$spaceLeft/$palletSize;
					$palletFullNotMet = 1;
				} else if($currentProductsCount == $palletSize && isset($vendorLimitNotMet)) {
					$infos['msg'] = $this->language->get('error_pallet_invalid_full');
					$infos['style'] = 'alert-warning';
					$pallet['infos'][] = $infos;
					$pallet['palletFullNotMet'] = 1;
				} else if(isset($palletLocked)) {
					$pallet['infos'][]['msg'] = $this->language->get('text_pallet_valid_locked');
					$pallet['palletFullNotMet'] = 0;
				} else {
				...
				// Validity Check
				if(!isset($vendorLimitNotMet) AND $pallet['palletFullNotMet'] == 0) {
					$pallet['valid'] = 1;
					$validator++;
				} else {
					$pallet['valid'] = 0;
				...
				$pallet['totals'] = array(
                	...
                	'value'  => $pallet_total_value
				);

				$pallet['size'] = $palletSize;
				$pallet['tab'] = $p + 1;
				$resultdata['pallets'][] = $pallet;
			}
			$total_data = array();
			$taxes = 0;
			...
			$shipping_country_id = $addresses[$shipping_address_id]['country_id'];
			...
			if($shipping = $this->model_pallet_worksheet->getShipping($worksheet_id)) {
				...
				if(isset($shipping_country_id)) {
					$shipping_by_country = $this->model_pallet_worksheet->getShippingByCountryId($shipping_country_id);
					if(!empty($shipping_by_country)) {
						foreach($shipping_by_country as $shipping_by_country_price) {
							$shipping_country[$shipping_by_country_price['key']] = $shipping_by_country_price['value'];
						}

						foreach ($shipping as $shipping_pallet) {
							$shipping_data['value'] += $shipping_country["pallets_shipping_x" . $shipping_pallet['pallet_qty'] . "_" . $shipping_pallet['pallet_size']];
				...
				$resultdata['shipping']['price'] = $this->currency->format($shipping_data['value']);
				$resultdata['shipping']['title'] = sprintf($this->language->get('text_worksheet_shipping'), $shipping_text);
			...
			$resultdata['totals'] = array();


            $data['currency'] = $this->currency;
			$data['selected_additional_currency'] = isset($_COOKIE["additional_currency"]) ? $_COOKIE["additional_currency"] : $this->currency->getCode();
			
			$data['additionalcurrencies'] = array();
			$this->load->model('localisation/currency');
			$currencies_temp = $this->model_localisation_currency->getCurrencies();
			foreach ($currencies_temp as $res) {
			...
			$data['ap_totals'] = array();

			foreach ($data['additionalcurrencies']as $total) {
				$data['ap_totals'][] = array(
					'code' => $total['code'],
					'title' => 'Total ' . $total['symbol_left'] . $total['symbol_right'],
					'text'  => $this->currency->format($this->currency->convert($worksheet_total, $this->currency->getCode(), $total['code']),$total['code'],1),
				);
			}
           ...
           'hide'=>'true',
			...
			if(count($palletIds) == $validator) {
				$resultdata['all_pallets_valid'] = '1';
			} else {
				$resultdata['all_pallets_valid'] = '';
			}

       		$resultdata['pallets_limit_v'] = $this->config->get("pallets_limit_v");
			$resultdata['continue'] = $this->url->link('common/home');

			$resultdata['checkout'] = $this->url->link('checkout/checkout', '', 'SSL');
			$this->response->setOutput(json_encode(array('status' => 'ok', 'data' => $resultdata)));
		}
					  ...
					  if (!$json) {
							...
							if($currentProductQty = $this->model_pallet_worksheet->getProductQty($pallet_id, $product_info['product_id'])) {
								...
								if (($woCount + $quantity <= $palletSize) && $stock_ok) {
                                    ...
                                    if($stock_ok)
									$json['error']['popup'] = sprintf($this->language->get('slider_error_pallet_limit'),$palletSize - $currentProductsCount,  $palletSize);
								}
							} else {
								if ($pallets_quantity+$quantity >= $product_info['quantity']){
	...
	public function slider_update() {
		...
		$quantity =  $this->request->post['quantity'];
		// Update
		if (isset($this->request->post['pallet_id']) && isset($this->request->post['product_id']) && isset($quantity) && $quantity > 0) {
			$this->load->model('pallet/worksheet');
			...
			if($this->model_pallet_worksheet->isPalletLocked($this->request->post['pallet_id'])) {
				$this->session->data['error'] = $this->language->get('error_pallet_locked');
				$json['error'] = $this->language->get('error_pallet_locked');
			} else {
                 $pallets_quantity = 0;
                 $pallets = $this->model_pallet_worksheet->getPallets($worksheet_id, $customer_id);
              ...
              $product_id= $this->request->post['product_id'];
                ...
                $stock_ok = true;
				 ...
				 $json['product_info'] = $product_info['quantity'];
				 $json['pallets_quantityquantity'] = $pallets_quantity+$quantity >= $product_info['quantity'];
				 $json['tq'] = $product_info['quantity']-$this->request->post['quantity'];
				...
				if ($pallets_quantity+$quantity >= $product_info['quantity']){
                      ...
                      if ($quantity_ < 0 ){
                          ...
                          $quantity = $product_info['quantity'];
                          $this->session->data['error'] = sprintf($this->language->get('error_pallet_stock'), $product_info['name'],$product_info['quantity']);
						  $json['error']  = sprintf($this->language->get('error_pallet_stock'), $product_info['name'],$product_info['quantity']);
						 $this->response->addHeader('Content-Type: application/json');
						 $this->response->setOutput(json_encode($json));
						 return;
                      }
				...
				if (($woCount + $quantity  <= $palletSize) && $stock_ok) {
                    ...
                    if ($stock_ok)
					  $this->session->data['error'] = sprintf($this->language->get('error_pallet_limit'), $palletSize);
					$json['error'] = sprintf($this->language->get('slider_error_pallet_limit'), $palletSize - $woCount, $palletSize);
				}
			}
			//$this->response->redirect($this->url->link('pallet/worksheet'));
		}
	...
	public function addpallet() {
		...
		if(isset($pallets)) {
			...
			{
            	$p = $this->model_pallet_worksheet->getPalletStatusData($pal, $customer_id);
				
				if($p['palletdata']['pallet_size'] != $p["qty"])
				{
					$allow = false;
					break;
				}
		...
		if($allow === true) {
			...
			if($pallets < $this->config->get('pallets_limit_c')) {
				...
				if($this->request->post['modal'] > 0) {
					...
					$json['success'] = sprintf($this->language->get('error_container_limit'), $this->config->get('pallets_limit_c'));
				} else {
			...
			if(isset($pallets)) {
				...
				$json['palletid'] = $pallets[count($pallets) - 1];
				foreach($pallets as $pal)
				{
					if($json['palletid'] != $pal)
						$this->model_pallet_worksheet->lockPallet($pal, $customer_id);
				}
			} else {
				$json['palletnumber'] = 0;
				$json['palletid'] = 0;
		...
		{
			$json['error']['popup'] = "You are not allowed to create additional pallet now!";
			$this->response->addHeader('Content-Type: application/json');
			$this->response->setOutput(json_encode($json));
		}
	}
		...
		if(isset($customer_id)) {
			...
			if(isset($pallet_id) && !$this->model_pallet_worksheet->isPalletLocked($pallet_id)) {
				...
				if($this->model_pallet_worksheet->hasProducts($pallet_id)) {
					...
					if(isset($products)) {
						foreach ($products as $product) {
							$pimage = $this->model_pallet_worksheet->getProductimage($product['product_id']);
							$this->load->model('tool/image');

						if ($pimage) {
							$pimage = $this->model_tool_image->resize($pimage, 100,100);
						} else {
							$pimage = $this->model_tool_image->resize('placeholder.png', 100,100);
              ...
              $price =  $this->currency->format($product['price']);
                        ...
                        if ($product['price'] != $fob_data['price']){
							...
							$pimage = "<img src='".$pimage."' style='width: 30px' class='seller-img'>";
	...
	public function checkvalid() {
		...
		if(isset($customer_id)) {
			...
			if(count($palletIds) == $validator) {
				...
				$json['all_pallets_valid'] = $validator;
				$data['all_pallets_valid'] = 1;
				$json['valid'] = 1;
	...
	public function destroyworksheet() {
		...
		if(isset($customer_id)) {
			...
			$this->session->data['error'] = $this->language->get('error_action_failed');
			$json['error'] = $this->language->get('error_action_failed');
		}
