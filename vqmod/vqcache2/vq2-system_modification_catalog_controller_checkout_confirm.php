...
class ControllerCheckoutConfirm extends Controller {
	public function index() {
		...
		if (!$redirect) {
            ...
            else $this->model_checkout_order->editOrder($this->session->data['order_id'], $order_data);
			...
			foreach ($this->cart->getProducts() as $product) {
                ...
                $full_total += $product['total'];
                
               $this->load->model('pallet/worksheet');
               $discount = $this->model_catalog_product->getProductDiscount($product['product_id'],$product['quantity']);
               $label = '';     
               ...
               if($discount['price'] !== null) {
                ...
                //    $this->model_pallet_worksheet->updateProductPrice($worksheet_id,$product_info, $customer_id);
                 ...
                 if ($product['price'] != $fob_data['price']){
                                ...
                                $label.'<span class="discounted_price">'.$this->currency->format($product['price']).'</span>';
			...
			foreach ($data['additionalcurrencies']as $total) {
				$data['ap_totals'][] = array(
					...
					'text'  => $this->currency->format($this->currency->convert($shipping+$full_total+$coupon, $this->session->data['currency'], $total['code']),$total['code'],1),
				);
			}
            ...
            $data['totals'][] = array(
					'title'=>'Total HK$',
                    'text' => $this->currency->format($this->currency->convert($shipping+$full_total+$coupon, $this->session->data['currency'], 'HKD'),"HKD",1),

           'hide'=>'true',
            
					//'text'=>$this->currency->getSymbolLeft('HKD').' '.round($this->currency->convert($shipping+$full_total+$coupon, $this->session->data['currency'], 'HKD'),0,PHP_ROUND_HALF_UP)
                    );
