...
// Load Vendor Language File MH 2017-02-07
            ...
            $langquery = $this->db->query("SELECT * FROM `" . DB_PREFIX . "language` WHERE language_id IN (SELECT `language_id` FROM `" . DB_PREFIX . "user` WHERE `user_id` = '" . (int)$vemail->row['user_id'] . "');");
...
// End Load Vendor Language File
            $language->load('mail/vendor_email_completed');
			         ...
			         if (floatval($fob['fob_price']) == floatval($fob['fob_bottle_full_price'])){
				        ...
				        $total = $this->currency->format($vendor_product['price']*$vendor_product['quantity'], $order_info['currency_code'], $order_info['currency_value']);
			        	 ...
			        	 $price = '<del>'.$this->currency->format(($fob['fob_bottle_full_price'] * $fob['pf']) + ($this->config->get('config_tax') ? $vendor_product['tax'] : 0), $order_info['currency_code'], $order_info['currency_value']).'</del>
                             <br />'.
                             ((strlen($vendor_product['obs'])>1)?$language->get('label_discount'):$language->get('label_special'))
      ...
      if ($vendor_products){
        ...
        foreach ($vendor_unique as $vendor) {
          if ($vendor['vendor_id']) {
            ...
            $language->load('mail/vendor_email_confirmation');
