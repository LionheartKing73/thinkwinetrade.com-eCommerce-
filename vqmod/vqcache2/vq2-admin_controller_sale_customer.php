...
class ControllerSaleCustomer extends Controller {
	...
	protected function getList() {
		...
		foreach ($results as $result) {
            ...
            $companies = $this->model_sale_customer->getCompanies($result['customer_id']);
            $company = '';
            if($companies !==false){
               foreach($companies as $com)
                 $company .= '<br><i>'.$com['company'].'</i>';
            }
            else
               $company = '';
        ...
        $data['sort_customer_id'] = $this->url->link('sale/customer', 'token=' . $this->session->data['token'] . '&sort=customer_id' . $url, 'SSL');
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_email']) || isset($this->request->get['filter_customer_id'])) {
			if (isset($this->request->get['filter_name'])) {
				$filter_name = $this->request->get['filter_name'];
			} else {
				$filter_name = '';
