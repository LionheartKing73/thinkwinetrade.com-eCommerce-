	...
	public function index() {
		...
		if ($product_info) {
			...
			$data['heading_title'] = $product_info['name'];
		           ...
		           $vendor_info = $this->db->query("SELECT vs.address_1 AS address_1, vs.address_2 AS address_2, vs.city AS city FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "vendor v ON (pd.product_id = v.vproduct_id) LEFT JOIN " . DB_PREFIX . "vendors vs ON (v.vendor = vs.vendor_id) WHERE p.product_id = '" . (int)$this->request->get['product_id'] . "'");

				   $location = $vendor_info->row['address_1'];
				   if($vendor_info->row['address_2']){
				   $location = $vendor_info->row['address_1'] ." ". $vendor_info->row['address_2'];
				   }
				   $region = $vendor_info->row['city'];
				   $latlong    =   $this->getLatLang($location, $region); 
		        $map        =   explode(',' ,$latlong);
		        $data['rvgmap'] = $map[0].", ".$map[1];
...
$data['entry_rvgmap'] = $this->language->get('entry_rvgmap');
            ...
            $data['text_discount_head_per_bottle'] = $this->language->get('text_discount_head_per_bottle');
			$data['text_discount_head_per_case'] = $this->language->get('text_discount_head_per_case');
...
$data['tab_rvgmap'] = $this->language->get('tab_rvgmap');
...
$this->load->model('pallet/worksheet');
	  ...
	  $data['vendor_id'] = $vendor_id;
       $data['vendor_rating'] = $this->model_pallet_worksheet->getTotalRatingsVendorsByVendorId($vendor_id);
			...
			foreach ($currencies_temp as $result) {
...
$data['pf'] = $product_info['pf'];
      $data['sp_price'] = $this->currency->format($this->tax->calculate($product_info['sp_price'], $product_info['tax_class_id'], $this->config->get('config_tax')));
			...
			foreach ($discounts as $discount) {
				...
				$res_disc['additional_currencies'] = array();
				foreach($data['additionalcurrencies'] as $curr)
				{
					$res_disc['additional_currencies'][] = array(
						'currency' => $curr,
						'price' => $this->currency->format($discount['price'] + ($this->config->get('config_tax') ? $product['tax'] : 0), $curr['code'] ),
						'price_bottle' => $this->currency->format($discount['sp_price'] + ($this->config->get('config_tax') ? $product['tax'] : 0), $curr['code'] )
					);
				}
				$data['discounts'][] = $res_disc;
			...
			$data['vendor_description'] = html_entity_decode($this->model_catalog_product->getVendorDescription($vendor_id), ENT_QUOTES, 'UTF-8');
...
$this->load->model('pallet/worksheet');
			...
			$results = $this->model_catalog_product->getVendorProductRelated($this->request->get['product_id']);
...
$pf = $result['pf'];
        $sp_price = $this->currency->format($this->tax->calculate($result['sp_price'], $result['tax_class_id'], $this->config->get('config_tax')));
...
$vendor = $this->model_catalog_product->getVendorByProduct($result['product_id']);
...
'vendor_rating'      => $vendor_ratingx,
			...
			/* START */
			$layout_positions = $this->load->controller('common/layout_product/layoutPositions');

			foreach($layout_positions as $position){
				$data[$position] = $this->load->controller('common/layout_product/index', $position);
			}
			/* END */
...
$data['price_bottom'] = $this->load->controller('common/price_bottom');
	...
	public function vendordescription()
    {
    	...
    	$json = array(
        	'html' =>  html_entity_decode($query->row['vendor_description'], ENT_QUOTES, 'UTF-8')
        );
