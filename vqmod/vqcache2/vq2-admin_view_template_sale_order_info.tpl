...
<?php } ?> <?php echo ($order_shipped ? '<button class="btn ' . ($product['documents_received'] ? 'alert-success" data-toggle="tooltip" data-original-title="'. $product['documents_received_date'] .'"' : 'alert-warning" onclick="confirmDocumentsReceived(this, '. $product['product_id'] .','. $product['pallet_id'] .');"') . '><i class="fa fa-files-o"></i></button>' : ''); ?></td>

			<td class="text-left"><small><a href="<?php echo $product['vhref']; ?>"><?php echo $product['vname']; ?></a> <?php echo ($product['vendor_confirmed'] ? '<span class="text-success">confirmed</span>' : ''); ?></small></td>
			
                  <td class="text-left"><?php echo $product['model']; ?></td>
                  <td class="text-right"><?php echo $product['quantity']; ?></td>
                  <td class="text-right"><?php echo $product['fob_price']; ?></td>
                  <td class="text-right"><?php echo $product['sp_price']; ?></td>
          ...
          <div class="tab-pane" id="tab-history">
            ...
            <fieldset>
              ...
              <form class="form-horizontal">
                <div class="form-group">
                  ...
                  <div class="col-sm-10">
                    <select name="order_status_id" id="input-order-status" class="form-control" <?php echo ($admin_confirmed ? 'disabled' : ''); ?>>
                ...
                <?php if(isset($button_action)) { ?>
                  <div class="form-group">
                    <div class="col-sm-offset-2 col-sm-10">
                      <button id="button-send-po" onclick="event.preventDefault();" data-loading-text="<?php echo $text_loading; ?>" class="btn btn-primary"><?php echo $button_action; ?></button>
                    </div>
                  </div>
                <?php } ?>
                <div class="form-group">
			...
			<div class="form-group">
              ...
              <div class="text-right">
                <button id="button-history" data-loading-text="<?php echo $text_loading; ?>" class="btn btn-primary" <?php echo ($admin_confirmed ? 'disabled' : ''); ?>><i class="fa fa-plus-circle"></i> <?php echo $button_history_add; ?></button>
              </div>
            </fieldset>
          </div>
          <?php if ($payment_action) { ?>
      ...
      <div class="order_total_big_monitor">
        <span>Total Commande: <?php echo $FOB_TOTAL?></span>
      </div>
    </div>
  </div>
...
function confirmProductReceived(btn, product_id, pallet_id) {
  $(btn).find('i').toggleClass('fa-spinner fa-spin');

  $.ajax({
    url: 'index.php?route=sale/order/confirmProductReceived&token=<?php echo $token; ?>&order_id=<?php echo $order_id; ?>&product_id=' + product_id + '&pallet_id=' + pallet_id,
    dataType: 'json',
    success: function(json) {
      ...
      setTimeout(function () {
        location.href = 'index.php?route=sale/order/info&token=<?php echo $token; ?>&order_id=<?php echo $order_id; ?>&tab=4';
      }, 1000);
    },
...
function confirmDocumentsReceived(btn, product_id, pallet_id) {
  $(btn).find('i').toggleClass('fa-spinner fa-spin');

  $.ajax({
    url: 'index.php?route=sale/order/confirmDocumentsReceived&token=<?php echo $token; ?>&order_id=<?php echo $order_id; ?>&product_id=' + product_id + '&pallet_id=' + pallet_id,
    dataType: 'json',
    success: function(json) {
      $('.alert').remove();
...
$('#button-history').on('click', function() {
  if(typeof verifyStatusChange == 'function'){
	...
	$.ajax({
        ...
        'append' : ($('input[name=\'append\']').prop('checked') ? 1 : 0),
        ...
        location.href = 'index.php?route=sale/order/info&token=<?php echo $token; ?>&order_id=<?php echo $order_id; ?>&tab=5';
			}
		},
		error: function(xhr, ajaxOptions, thrownError) {
			alert(thrownError + "\r\n" + xhr.statusText + "\r\n" + xhr.responseText);
...
$('#button-send-po').on('click', function() {
  $.ajax({
    url: 'index.php?route=sale/order/api&token=<?php echo $token; ?>&api=api/order/history&order_id=<?php echo $order_id; ?>',
    type: 'post',
    dataType: 'json',
    data: 'send_po=1&order_status_id=' + encodeURIComponent($('select[name=\'order_status_id\']').val()) + '&notify=' + ($('input[name=\'notify\']').prop('checked') ? 1 : 0) + '&append=' + ($('input[name=\'append\']').prop('checked') ? 1 : 0) + '&comment=' + encodeURIComponent($('textarea[name=\'comment\']').val()),
    beforeSend: function() {
      $("#button-send-po").html('<i class="fa fa-spinner fa-spin"></i> <?php echo $button_action; ?>');
		},
    success: function(json) {
      $('.alert').remove();
