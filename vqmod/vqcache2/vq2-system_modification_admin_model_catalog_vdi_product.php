...
if($data['fob_price']>0) { $fob_price = $data['fob_price']; } else { $fob_price=0; }
	...
	//END
        ...
        foreach ($data['product_attribute'] as $product_attribute) {
                if ($product_attribute['attribute_id']) {
                    ...
                    if(empty($product_attribute['attribute_description'])) continue;
                    $product_attribute_description = $product_attribute['attribute_description'];

                    if (isset($product_attribute['attribute_type']) && ($product_attribute['attribute_type'] == "multiple" || $product_attribute['attribute_type'] == "select")) {
                        foreach($languages as $language) {
                            $attr = array();
                            foreach($product_attribute_description as $description) {
                                $values = $this->model_catalog_attribute->getAttributeValueDescriptions($description);
                            if(isset($values[$language['language_id']]['name'])) $attr[] = $values[$language['language_id']]['name'];
                            } 

                            $attr2[$language['language_id']]['text'] = implode(", ", $attr);
                        }

                        $product_attribute['product_attribute_description'] = $attr2;
                    } else {
                    
                        foreach($languages as $language) {
                            $attr[$language['language_id']]['text']  = $product_attribute_description;
                        }
                        
                        $product_attribute['product_attribute_description'] = $attr;
                    }
                    // End
	...
	/*	if (isset($data['product_discount'])) {
        ...
        if (isset($data['product_discount'])) {
            if (intval($data['product_discount'][0]['fob_price']) != 0){
			
            $this->load->model('sale/customer_group');

            $customer_groups = $this->model_sale_customer_group->getCustomerGroups();
         
           $all_groups = array(); $i=0;
           foreach ($data['product_discount'] as $dd){ 
           ...
           if ((int)$all_groups[0]['fob_price'] != 0)
 		   foreach ($all_groups as $i=>$product_discount) {
				 if($product_discount['fob_price']>0) { $fob_price = $product_discount['fob_price']; } 
                 else { 
                    $product_discount['fob_price'] = $all_groups[$i-1]['fob_price'];
                    $all_groups[$i]['fob_price'] = $all_groups[$i-1]['fob_price'];
                    $fob_price=$all_groups[$i-1]['fob_price']; 
                 }
                ...
                $sql = "INSERT INTO " . DB_PREFIX . "product_discount SET product_id = '" .
                              (int)$product_id . "', customer_group_id = '" . (int)$product_discount['customer_group_id'] . 
                              "', quantity = '" . (int)$product_discount['quantity']."', priority = '" . (int)$product_discount['priority'] . "', fob_price = '" . $fob_price .
                               "', sp_price = '" . $sp_price. "', `step-text` = '" . $product_discount['step-text']."', price = '" . $price . "', date_start = '" .
 ...
 ;
    ...
    $price = $data['pf'] * $sp_price;
      ...
      $this->db->query("INSERT INTO " . DB_PREFIX . "product_special SET product_id = '" . (int)$product_id . "', customer_group_id = '" . (int)$product_special['customer_group_id'] . "', priority = '" . (int)$product_special['priority'] . "', fob_price = '" . $fob_price . "', sp_price = '" . $sp_price. "', price = '" . $price . "', date_start = '" . $this->db->escape($product_special['date_start']) . "', date_end = '" . $this->db->escape($product_special['date_end']) . "'");
			}
		}
	...
	//END
		...
		if (isset($data['keyword'])) {
            $keyword = str_replace(array(" ","'","/","]"),"-",$data['keyword']);
		...
		if(!empty($data['customRating']))
		{
			foreach($data['customRating'] as $key => $rate)
			{
				$rating = $this->db->query("SELECT type FROM ".DB_PREFIX."ratings WHERE id='{$rate}'" );
				if($rating->row['type'] == 'award_places')
				{
					$sq = "value_id = '{$data['ratingValue'][$key]}'";
				} else {
					$sq = "value = '{$data['ratingValue'][$key]}'";
				}

				$this->db->query("INSERT INTO ". DB_PREFIX ."product_ratings SET product_id='$product_id',rating_id='{$rate}', $sq" );
			}                    
		}
 ...
 ;
	...
	//END
		...
		if(!empty($data['customRating']))
		{
			$this->db->query("DELETE FROM ".DB_PREFIX."product_ratings WHERE product_id='$product_id'");
			$sql = array();
			foreach($data['customRating'] as $key => $rate)
			{
				$rating = $this->db->query("SELECT type FROM ".DB_PREFIX."ratings WHERE id='{$rate}'" );
				...
				$sql[] = "INSERT INTO ". DB_PREFIX ."product_ratings SET product_id='$product_id',rating_id='{$rate}', $sq";
        ...
        $this->event->trigger('post.fob.save',$data);
    }
 ...
 /**************/   
	...
	public function getProductSpecials($product_id, $language_id=2) {
		//$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_special WHERE product_id = '" . (int)$product_id . "' ORDER BY priority, price");
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_special p LEFT JOIN " . DB_PREFIX . "customer_group_description d ON d.customer_group_id = p.customer_group_id WHERE product_id =".$product_id." AND language_id = ".$language_id." ORDER BY priority, price");
