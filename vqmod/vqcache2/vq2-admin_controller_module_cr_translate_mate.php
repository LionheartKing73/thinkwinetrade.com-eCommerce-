...
class ControllerModuleCrTranslateMate extends Controller{
    ...
    public function index() {
        ...
        $data = $this->load->language('module/'.$this->modName);
        ...
        $data['breadcrumbs'] = array(
            ...
            array( // This module
                ...
                'href' => $this->url->link('module/'.$this->modName, 'token=' . $this->session->data['token'], 'SSL')
            ),
        );
        ...
        $this->response->setOutput($this->load->view('module/'.$this->modName.'.tpl', $data));
    }
    ...
    public function ajaxLoad($data) {
		...
		$opts = array();
    	if ( !empty($_GET['length']) ) { $opts['length'] = (int)$_GET['length']; }
    	if ( !empty($_GET['startAfter']) ) { $opts['startAfter'] = (string)$_GET['startAfter']; }
    	if ( !empty($_GET['singleFile']) ) { $opts['singleFile'] = $_GET['singleFile'] === '' ? FALSE : (string)$_GET['singleFile']; }
    	if ( !empty($_GET['keyFilter']) ) { $opts['keyFilter'] = $_GET['keyFilter'] === '' ? FALSE : (string)$_GET['keyFilter']; }
    	if ( !empty($_GET['textFilter']) ) { $opts['textFilter'] = $_GET['textFilter'] === '' ? FALSE : (string)$_GET['textFilter']; }
    	if ( !empty($_GET['dirKey']) ) { $opts['dirKey'] = (string)$_GET['dirKey']; }
    	if ( !empty($_GET['notTranslated']) ) { $opts['notTranslated'] = $_GET['notTranslated'] == 'true'; }
    ...
    protected function returnAjaxErrors($errors) {
    	...
    	<?php }
	    	...
	    	foreach ( $errors as $error ) { var_dump($error); die(); ?>
    ...
    protected function userHasPermission($permission) {
    	return $this->user->hasPermission($permission, 'module/'.$this->modName);
    }
