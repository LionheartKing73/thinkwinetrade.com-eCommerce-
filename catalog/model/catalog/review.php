...
class ModelCatalogReview extends Model {
	public function addReview($product_id, $data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "review SET order_id = '" . $this->db->escape($data['order_id']) . "', author = '" . $this->db->escape($data['name']) . "', customer_id = '" . (int)$this->customer->getId() . "', product_id = '" . (int)$product_id . "', text = '" . $this->db->escape($data['text']) . "', rating = '" . (int)$data['rating'] . "', date_added = NOW()");
	...
	public function getReviewByOrderId($order_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "review r WHERE r.order_id = '".$order_id."' group by r.product_id" );
		$reviews = array();
		foreach($query->rows as $review){
			$arr = $review['product_id'];
			$reviews[$arr]['product_id'] = $review['product_id'];
			$reviews[$arr]['text'] = $review['text'];
			$reviews[$arr]['rating'] = $review['rating'];
		}
		return $reviews;
	}
}