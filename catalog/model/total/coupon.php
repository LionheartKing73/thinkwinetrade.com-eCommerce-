...
class ModelTotalCoupon extends Model {
    ...
    public function getWTotal(&$total_data, &$total, &$taxes, $worksheet_id) {
		...
		if (isset($this->session->data['coupon'])) {
			...
			$coupon_info = $this->model_checkout_coupon->getWCoupon($this->session->data['coupon'], $worksheet_id);
			...
			if (!$coupon_info['product']) {
						...
						$sub_total =  $this->model_pallet_worksheet->getWorksheetTotals($worksheet_id);
  
				} else {
							$sub_total = 0;
                            
							foreach ($this->model_pallet_worksheet->getWorksheetProducts($worksheet_id) as $product) {
								if (in_array($product['product_id'], $coupon_info['product'])) {
									$sub_total += $product['total'];
								}
							}
