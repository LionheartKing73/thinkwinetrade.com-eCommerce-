...
class ModelPalletWorksheet extends Model {
	public function getWorksheet($customer_id) {


		$worksheetId = $this->db->query("SELECT worksheet_id FROM " . DB_PREFIX . "worksheet WHERE customer_id = '" . (int)$customer_id . "'  AND order_id IS NULL");

    	if(isset($worksheetId->row['worksheet_id'])) {
			return $worksheetId->row['worksheet_id'];
		} else {
			return $this->addWorksheet($customer_id);
		}
	}


	private function addWorksheet($customer_id) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "worksheet SET customer_id = '" . (int)$customer_id . "', date_added = NOW(), date_modified = NOW()");
		return $this->db->getLastId();
	}

  	public function archiveWorksheet($order_id, $worksheet_id, $customer_id) {
      	$this->db->query("UPDATE " . DB_PREFIX . "worksheet SET archived = '1', order_id = '" . (int)$order_id . "', date_modified = NOW() where worksheet_id = '" . (int)$worksheet_id . "' AND customer_id = '" . (int)$customer_id . "'");
	}

	public function getPalletOld($worksheet_id, $customer_id) {
	...
	public function getPallet($worksheet_id, $customer_id) {
		$palletId = $this->db->query("SELECT pallet_id FROM " . DB_PREFIX . "pallet WHERE worksheet_id = '" . (int)$worksheet_id . "' AND customer_id = '" . (int)$customer_id . "' ORDER BY pallet_id DESC");
		if(isset($palletId->row['pallet_id'])) {
			return $palletId->row['pallet_id'];
		} /*else if (!$this->hasPallets($worksheet_id)) {
			return $this->addPallet($worksheet_id, $customer_id);
	...
	public function addPallet($worksheet_id, $customer_id, $pallet_size) {
		//$this->db->query("UPDATE " . DB_PREFIX . "pallet SET locked='1' WHERE worksheet_id = '" . (int)$worksheet_id . "' AND customer_id = '" . (int)$customer_id . "'");
		$this->db->query("INSERT INTO " . DB_PREFIX . "pallet SET worksheet_id = '" . (int)$worksheet_id . "', customer_id = '" . (int)$customer_id . "', pallet_size = '" . (int)$pallet_size . "', date_added = NOW(), date_modified = NOW()");
		return $this->db->getLastId();
	}
	...
	public function lockAllPallets($worksheet_id, $customer_id) {
		$this->db->query("UPDATE " . DB_PREFIX . "pallet SET locked='1' WHERE worksheet_id = '" . (int)$worksheet_id . "' AND customer_id = '" . (int)$customer_id . "'");
	}
	...
	//public function generatePalletNo($order_id, $pallet_by_order, $customer_id) {
	public function generateArchiveNos($order_id, $pallet_by_order, $customer_id, $pallet_size) {
		...
		$nos = array();
		//$nos['pallet'] = "PALL-".$customer_id."-".$initials."-".$date."-".$order_id."-".$pallet_by_order;
	...
	public function getCustomerName($customer_id) {
		$customer = $this->db->query("SELECT CONCAT(firstname, ' ', lastname) AS customer_name FROM " . DB_PREFIX . "customer WHERE customer_id = '" . (int)$customer_id . "'");
	...
	public function destroyPallet($pallet_id, $customer_id) {
		$this->db->query("DELETE FROM " . DB_PREFIX . "pallet_product WHERE pallet_id = '" . (int)$pallet_id . "'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "pallet WHERE pallet_id = '" . (int)$pallet_id . "' AND customer_id = '" . (int)$customer_id . "'");
	}

	public function destroyWorksheet($customer_id) {

		$missing_order = $this->db->query("SELECT order_id FROM " . DB_PREFIX . "order WHERE customer_id = '" . (int)$customer_id . "' AND  order_status_id=0 limit 0,1");

		if (count($missing_order->row)){
		   $order_id = $missing_order->row['order_id'];
		   //$this->deleteOrder($order_id);
		   $this->db->query("update " . DB_PREFIX . "pallet_product set order_id = null where order_id='" . (int)$order_id . "'");
		   $this->db->query("update " . DB_PREFIX . "pallet set order_id = null where order_id='" . (int)$order_id . "' AND customer_id = '" . (int)$customer_id . "'");
		   $this->db->query("update " . DB_PREFIX . "worksheet set order_id = null where order_id='" . (int)$order_id . "' AND customer_id = '" . (int)$customer_id . "'");
		}
		...
		foreach($pallets->rows as $row) {
			$this->db->query("DELETE FROM " . DB_PREFIX . "pallet_product WHERE pallet_id = '" . (int)$row['pallet_id'] . "' and order_id is NULL");
		}
		$this->db->query("DELETE FROM " . DB_PREFIX . "pallet WHERE customer_id = '" . (int)$customer_id . "' and order_id is NULL");
	    $this->db->query("DELETE FROM " . DB_PREFIX . "worksheet WHERE customer_id = '" . (int)$customer_id . "' and order_id is NULL");


	}


	public function getPallets($worksheet_id, $customer_id) {
		$pallets = [];
		$palletIds = $this->db->query("SELECT pallet_id FROM " . DB_PREFIX . "pallet WHERE worksheet_id = '" . (int)$worksheet_id . "' AND customer_id = '" . (int)$customer_id . "' ORDER BY pallet_id");
	...
	public function getProductAttributes($product_id, $language) {
		$products = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_attribute as attr_t WHERE product_id = '" . (int)$product_id . "' AND language_id = '" . (int)$language . "'");
	...
	public function getProducts($pallet_id) {
		$products = $this->db->query("SELECT ppd.*, prd.pf FROM " . DB_PREFIX . "pallet_product as ppd LEFT JOIN  " . DB_PREFIX . "product as prd on prd.product_id = ppd.product_id WHERE pallet_id = '" . (int)$pallet_id . "' order by ppd.pallet_product_id");
		return $products->rows;
	}

	public function getProductimage($product_id) {
		$product = $this->db->query("SELECT image FROM " . DB_PREFIX . "product WHERE product_id = '" . (int)$product_id . "'");
		return $product->row['image'];
	}

    public function getProduct($pallet_id, $product_id) {
		$products = $this->db->query("SELECT * FROM " . DB_PREFIX . "pallet_product WHERE pallet_id = '" . (int)$pallet_id . "' and product_id='".$product_id."'");
		return $products->row;
	}
	...
	public function getPalletStatusData($pallet_id, $customer_id) {
		$data = array();
		$palletData = $this->db->query("SELECT * FROM " . DB_PREFIX . "pallet WHERE pallet_id = '" . (int)$pallet_id . "'");
		if(isset($palletData->row)) {
			$data['palletdata'] = $palletData->row;
		}
	...
	public function addToPallet($pallet_id, $product, $qty = 1) {
	...
	public function getVendor($product_id) {
        ...
        if(isset($getVenId->row['vendor_id'])){
              return $getVenId->row['vendor_id'];
			  }
	}

		public function getVendorcountry($product_id) {
		$getVenName = $this->db->query("SELECT c.iso_code_2 AS iso_code_2 FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "vendor v ON (p.product_id = v.vproduct_id) LEFT JOIN " . DB_PREFIX . "vendors vs ON (v.vendor = vs.vendor_id) LEFT JOIN " . DB_PREFIX . "country c ON (c.country_id = vs.country_id) WHERE p.product_id = '" . (int)$product_id . "'");

		return $getVenName->row['iso_code_2'];
	}

	public function getVendorName($product_id) {
		$getVenName = $this->db->query("SELECT vs.vendor_name AS vendor FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "vendor v ON (p.product_id = v.vproduct_id) LEFT JOIN " . DB_PREFIX . "vendors vs ON (v.vendor = vs.vendor_id) WHERE p.product_id = '" . (int)$product_id . "'");

		return $getVenName->row['vendor'];
	}

	public function getProductNameById($product_id) {
		$result = $this->db->query("SELECT name AS product_name FROM " . DB_PREFIX . "product_description WHERE product_id = '" . (int)$product_id . "'");
		return $result->row;
	}
	...
	public function updateTotals($pallet_id) {
		...
		foreach ($products->rows as $product) {
			...
			$this->db->query("UPDATE " . DB_PREFIX . "pallet_product SET total = '" . (float)$total . "' WHERE pallet_id = '" . $pallet_id . "' AND product_id = '" . (int)$product['product_id'] . "'");
		}
	}
	...
	public function getShipping($worksheet_id) {
		...
		$pallets = $this->db->query("SELECT pallet_size, count(*) as pallet_qty FROM " . DB_PREFIX . "pallet WHERE worksheet_id = '" . $worksheet_id . "' GROUP BY pallet_size");
		//SELECT pallet_size, count(*) as pallet_qty FROM `oc_pallet` WHERE worksheet_id='75' group by pallet_size
		return $pallets->rows;
	}

	public function getShippingByCountryId($country_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "pallet_shipping WHERE country_id = '" . (int)$country_id . "'");
	...
	public function getOrderProducts($order_id) {
		$query = $this->db->query("SELECT op.*, p.sku FROM " . DB_PREFIX . "order_product op LEFT JOIN " . DB_PREFIX . "product p ON (op.product_id = p.product_id) WHERE op.order_id = '" . (int)$order_id . "'");

		return $query->rows;
	}

	public function addToPalletProducts($pallet_id, $order_id,$products) {
    ...
    public function getWorksheetProducts($worksheet_id){
        $query = $this->db->query("SELECT * FROM `oc_pallet_product` pp  left join oc_pallet p on p.pallet_id=pp.pallet_id WHERE worksheet_id=".$worksheet_id);
    ...
    function updateProductPrice($worksheet_id,$product_info,$customer_id){
         $query = $this->db->query("select * from `" . DB_PREFIX . "pallet_product` pp where product_id=".$product_info['product_id']." and
                  pallet_id in (select pallet_id from oc_pallet where worksheet_id=".$worksheet_id." and
                  customer_id=".$customer_id.")");

         $total_quantity_of_this_product  = $this->getWorksheetProductQty($worksheet_id, $product_info['product_id']);
         $discount = $this->model_catalog_product->getProductDiscount($product_info['product_id'],$total_quantity_of_this_product);

         foreach ($query->rows as $row){

            $price = $product_info['price'];
            if ($product_info['special'] !== null)
           ...
           $this->db->query("update `".DB_PREFIX . "pallet_product`
                    set `price`=".$price.", `total`=".$price*$row['quantity']." where pallet_product_id=".$row['pallet_product_id']);
         }
    }

    /**zighia*/
    public function getTotalRatingsVendorsByVendorId($vendors) {
      	//$query = $this->db->query("SELECT COUNT(*) as total_ratings FROM  " . DB_PREFIX . "rating WHERE ((rating =1 AND (STATUS =1 OR STATUS =0)) OR (rating = 2 AND STATUS =1)) AND vendor_id = '". (int)$vendors  ."'");
		$query = $this->db->query("SELECT COUNT(*) as total_ratings FROM  " . DB_PREFIX . "rating WHERE STATUS =1 AND vendor_id = '". (int)$vendors  ."'");
		$total_ratings = $query->row['total_ratings'];
		$query1 = $this->db->query("SELECT COUNT(*) as total FROM  " . DB_PREFIX . "rating WHERE rating =1 AND STATUS =1 AND vendor_id = '". (int)$vendors  ."'");
		$total = $query1->row['total'];
		$percentage = round(($total/$total_ratings)*100,2);
		//$rating = $total.'/'.$total_ratings.' ('.$percentage.'%)';
		$rating = $total.'/'.$total_ratings.' ('.$percentage.'%)';
		return $rating;
	}
