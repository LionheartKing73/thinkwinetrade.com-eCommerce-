...
class ModelAccountSignUp extends Model {
	...
	public function addVendorSignUp($data) {
            //2017-02-07 Modified by Mark Horton to save current language to user table
                $language_id = $this->db->query("SELECT IFNULL(`language_id`, 1) FROM `" . DB_PREFIX . "language` WHERE `code` = '" . $this->session->data['language'] . "';");
		$this->db->query("INSERT INTO `" . DB_PREFIX . "user` SET username = '" . $this->db->escape($data['username']) . "', password = '" . $this->db->escape(md5($data['password'])) . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', folder = '" . $this->db->escape($data['username']) . "', language_id = '" . $language_id . "', user_group_id = '50', date_added = NOW()");
	...
	public function deleteEmailInactiveBySignUp($email,$username) {//Th-add
		$query = $this->db->query("DELETE FROM `" . DB_PREFIX . "user` WHERE (email = '" . $this->db->escape($email) . "' OR username = '" . $this->db->escape($username) . "' ) AND status = 5");
	}
	...
	public function deleteUserID($user_id) {//Th-add
		$query = $this->db->query("DELETE FROM `" . DB_PREFIX . "user` WHERE user_id = '" . (int)$this->db->escape($user_id) . "'");

		$query1 = $this->db->query("DELETE FROM `" . DB_PREFIX . "vendors` WHERE user_id = '" . (int)$this->db->escape($user_id) . "'");

	}

	public function addVendorSignUpPlan_PayPal($data) {//Th-add custom from addVendorSignUp()
            //2017-02-07 Modified by Mark Horton to save current language to user table
                $language_id = $this->db->query("SELECT IFNULL(`language_id`, 1) FROM `" . DB_PREFIX . "language` WHERE `code` = '" . $this->session->data['language'] . "';");
		$this->db->query("INSERT INTO `" . DB_PREFIX . "user` SET username = '" . $this->db->escape($data['username']) . "', password = '" . $this->db->escape(md5($data['password'])) . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', folder = '" . $this->db->escape($data['username']) . "', language_id = '" . $language_id . "', user_group_id = '50', date_added = NOW()");
		//$this->db->query("INSERT INTO `" . DB_PREFIX . "user` SET username = '" . $this->db->escape($data['username']) . "', password = '" . $this->db->escape(md5($data['password'])) . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', user_group_id = '50', date_added = NOW()");
	...
	public function sendMailSignUpCompletePayment($user_id){
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "signup_fee_history` WHERE user_id = '" . (int)$user_id . "'");
		$signup_plan = $query->row;
		...
		/*if ($this->config->get('mvd_signup_auto_approval')) {
			...
			if ($signup_plan['commission_type'] == '4' || $signup_plan['commission_type'] == '5') {
				$message .= $this->language->get('text_login') . "<br><br>";
			} else {
				$message .= $this->language->get('text_approval') . "<br><br>";
			}
		...
		$message .= $this->language->get('text_signup_information') . "<br>" ;
		$message .= $this->language->get('text_signup_username') . $user['username'] . "<br>";
		$message .= '<font >' . $this->language->get('text_signup_plan') . $signup_plan['signup_plan'] . "<br>";
		$message .= $this->language->get('text_signup_date') . $signup_plan['user_date_start'] . "<br>";

		$message .= $this->language->get('text_signup_expire_date') . $signup_plan['user_date_end'] . "<br>";
		...
		/*trigger to store admin*/
		$subject_join = sprintf($this->language->get('text_subject1'), $user['username']);

		$text = sprintf($this->language->get('text_to'), $this->config->get('config_owner')) . "<br><br>";
		$text .= sprintf($this->language->get('text_join'), $user['username'], $signup_plan['vendor_name']) . "<br><br>";
		$text .= $this->language->get('text_thanks') . "<br>";
		$text .= $this->config->get('config_name');
