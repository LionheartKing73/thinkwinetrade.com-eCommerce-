...
<html lang="<?php echo $lang; ?>">
    <head>
        ...
        <style type="text/css">
			#linkedin-login-userdetails {
				margin: 20px;	
			}
			
			#linkedin-login-userdetails form > div {
				margin: 5px;	
			}
			
			#linkedin-login-userdetails form > div.error {
				margin-bottom: 10px;	
			}
			
			#linkedin-login-userdetails form > div#linkedin-info {
				margin-bottom: 20px;	
			}
		</style>
        ...
        <script type="text/javascript">
            ...
            <?php if ($has_customer_group) : ?>
				<?php if ($enabled['region']) : ?>
					$('#linkedin-login-userdetails select[name=\'zone_id\']').parent().hide();
				<?php endif; ?>
                ...
                <?php if (count($customer_groups) > 1) { ?>
                    $('#linkedin-login-userdetails input[name=\'customer_group_id\']:checked').on('change', function() {
                        triggerCustomerGroupDependentFields($(this).val());
                    });
                    $('#linkedin-login-userdetails input[name=\'customer_group_id\']:checked').trigger('change');
                    <?php } else { ?>
                    triggerCustomerGroupDependentFields($('#linkedin-login-userdetails input[name=\'customer_group_id\']').val());
                    <?php } ?>
                <?php endif; ?>
				...
				var triggerZoneField = function(value) {
					$.ajax({
						url: 'index.php?route=account/linkedinlogin/country&country_id=' + value,
						dataType: 'json',
						beforeSend: function() {
							if ($('#linkedin-login-userdetails select[name=\'country_id\']').length > 0) {
								$('#linkedin-login-userdetails select[name=\'country_id\']').after('<span class="wait">&nbsp;<i class="fa fa-spinner fa-spin"></i></span>');
							}
							<?php if ($enabled['region']) : ?>
								$('#linkedin-login-userdetails select[name=\'zone_id\']').parent().slideUp(100);
							<?php endif; ?>
						},
						complete: function() {
							$('.wait').remove();
							<?php if ($enabled['region']) : ?>
								$('#linkedin-login-userdetails select[name=\'zone_id\']').parent().slideDown();
							<?php endif; ?>
						},			
						success: function(json) {
							...
							$('#linkedin-login-userdetails select[name=\'zone_id\']').html(html);
						},
				...
				$('#linkedin-login-userdetails select[name=\'country_id\']').bind('change', function() {
					triggerZoneField($(this).val());
				});
				triggerZoneField($('#linkedin-login-userdetails select[name=\'country_id\']').val());
				<?php } else { ?>
				triggerZoneField($('#linkedin-login-userdetails input[type=\'hidden\'][name=\'country_id\']').val());
				<?php } ?>
				...
				$('#button-submit').on('click', function(e) {
					...
					$.ajax({
						url: 'index.php?route=account/linkedinlogin/validate&module_id='+module_id,
						type: 'post',
						data: fields,
						dataType: 'json',
						beforeSend: function() {
							...
							$('#button-submit').after('<span class="wait">&nbsp;<i class="fa fa-spinner fa-spin"></i></span>');
						},	
						complete: function() {
						...
						success: function(json) {
							...
							if (json['error']) {
								...
								$(fields).attr('disabled', false);
								$('#linkedin-login-userdetails form').submit();
							}
						},
						error: function(xhr, ajaxOptions, thrownError) {
							console.log(thrownError + "\r\n" + xhr.statusText + "\r\n" + xhr.responseText);
    ...
    <body>
    	<div id="linkedin-login-userdetails">
            <form method="POST" action="<?php echo $submit_url; ?>">
                <div id="linkedin-info"><?php echo $text_your_details; ?></div>
