      ...
      if (json['error']) {
        if (json['error']['popup']) {
         ...
         /* $(".alert").fadeTo(2000, 500).slideUp(500, function(){
	       ...
	       $('#contentnot').after('<div class="topbar topbar-warning" id="mynotification3"><div class="top_bar_padding">' + json['error']['popup'] + '<button type="button" class="close" data-dismiss="message">&times;</button></div>');
      ...
      success: function(json) {
        ...
        if (json['error']) {
          ...
          //  $('.breadcrumb').after('<div class="alert alert-danger">' + json['error']['popup'] + '<button type="button" class="close" data-dismiss="alert">&times;</button></div>');
	       ...
	       $('#contentnot').after('<div class="topbar topbar-danger" id="mynotification3" style="margin-bottom:10px"><div class="top_bar_padding">' + json['error']['popup'] + '<button type="button" class="close" data-dismiss="message">&times;</button></div>');
		   $("#mynotification3").topBar({slide: true})
        ...
        if (json['success']) {
          ...
          if (json['products']) {
            ...
            if (json['valid']) {
              ...
              html += '  <div class="progress-bar progress-bar-success" role="progressbar" aria-valuenow="' + json['current'] + '" aria-valuemin="0" aria-valuemax="' + json['progress']['limit'] + '" style="width: ' + json['progress']['current'] + '%;">';
              html += '    ' + json['space']['current'] + ' / ' + json['progress']['limit'];
              html += '  </div>';
              html += '</div>';

            } else {
              html += '<div class="progress">';
              html += '  <div class="progress-bar" role="progressbar" aria-valuenow="' + json['progress']['current'] + '" aria-valuemin="0" aria-valuemax="' + json['progress']['limit'] + '" style="width: ' + json['progress']['current'] + '%;">';
              html += '    ' + json['space']['current'] + ' / ' + json['progress']['limit'];
              html += '  </div>';
              html += '  <div class="progress-bar progress-bar-danger" style="width: ' + json['progress']['left'] + '%">';
              html += '    ' + json['space']['left'] + ' ' + json['text_space_left'];
              html += '  </div>';
              html += '</div>';
            }
            html += '</td>';
            html += '  <td colspan="3" class="text-right"><b>' + json['totals']['title'] + '</b></td>';
            html += '  <td class="text-right"><b>' + json['totals']['text'] + '</b></td>';
            html += '</tr>';
          } else {
            html += '<tr>';
            html += '  <td>-</td>';
  ...
  'checkvalid': function() {
    ...
    $.ajax({
      ...
      success: function(json) {
        ...
        html = '';
		if (json['valid']) {
          //html += '<p class="valid-modal pull-right"><a class="btn btn-default" href="#" onclick="worksheet.addpallet(1); return false;"><i class="fa fa-plus-square"></i> ' + json['create_pallet'] + '</a> or ';
