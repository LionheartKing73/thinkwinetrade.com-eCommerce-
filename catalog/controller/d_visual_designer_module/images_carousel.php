...
class ControllerDVisualDesignerModuleImagesCarousel extends Controller {
	...
	public function index($setting){
		...
		if(isset($data['setting']['images'])){
			$images_data = $data['setting']['images'];	
		}
		else{
			$images_data = array();
		}
		...
		$links = explode(PHP_EOL, $data['setting']['link']);
		...
		foreach ($images_data as $value) {
			...
			if(!empty($links[$count_link])){
				$link = $links[$count_link];
			}
			else{
				$link = '';
			}
