...
class ControllerProductCompare extends Controller {
	public function index() {
        ...
        $data['text_price_bottle'] = $this->language->get('text_price_bottle');
		...
		foreach ($this->session->data['compare'] as $key => $product_id) {
			...
			if ($product_info) {
				...
				if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
                    ...
                    $price_bottle = $this->currency->format($this->tax->calculate($product_info['price']/$product_info['pf'], $product_info['tax_class_id'], $this->config->get('config_tax')));
                     $hk = $this->currency->getSymbolLeft('HKD').' '.round($this->currency->convert($product_info['sp_price'], 'EUR', 'HKD'),0,PHP_ROUND_HALF_UP);
				} else {
					$price = false;
                    $price_bottle = false;
                    $hk = false;
				...
				if ((float)$product_info['special']) {
                    ...
                    $special_bottle = $this->currency->format($this->tax->calculate($product_info['special']/$product_info['pf'], $product_info['tax_class_id'], $this->config->get('config_tax')));
                    $special_hk = $this->currency->getSymbolLeft('HKD').' '.round($this->currency->convert($product_info['special']/$product_info['pf'], 'EUR', 'HKD'),0,PHP_ROUND_HALF_UP);
				} else {
					$special = false;
                    $special_bottle = false;
                    $special_hk  = false;
				}
				...
				$data['products'][$product_id] = array(
                    ...
                    'hk'           => $hk,
					'special'      => $special,
                    'special_hk'   => $special_hk,
                    'special_bottle'      => $special_bottle,
					'description'  => utf8_substr(strip_tags(html_entity_decode($product_info['description'], ENT_QUOTES, 'UTF-8')), 0, 200) . '..',
					'model'        => $vendor, //$product_info['model'],
	...
	public function add() {
		...
		if ($product_info) {
			if (!in_array($this->request->post['product_id'], $this->session->data['compare'])) {
				if (count($this->session->data['compare']) >= 6) {
