    ...
    public function index($setting) {
        ...
        if($this->{'model_module_'.$this->codename}->validateEdit($setting['config'])){
            ...
            if (file_exists(DIR_TEMPLATE . $this->theme . '/javascript/d_visual_designer.js')) {
                $this->scripts[] = 'catalog/view/theme/' . $this->theme . '/javascript/d_visual_designer.js?'.$this->extension['version'];
            } else {
                $this->scripts[] = 'catalog/view/theme/default/javascript/d_visual_designer.js?'.$this->extension['version'];
            }
            $_GET['vd'] = 1;
            }
    ...
    public function getBlocks(){
        ...
        if(isset($level)){
            ...
            foreach ($results as $block) {
                ...
                $json['notify'] = $this->{'model_module_'.$this->codename}->checkCompleteVersion();
