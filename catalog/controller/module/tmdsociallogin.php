...
class ControllerModuletmdSociallogin extends Controller {
	public function index($setting) {
		...
		{
				if($this->request->get['route']=='common/quicklogin')
				{
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/module/tmdsociallogin.tpl')) {
			return $this->load->view($this->config->get('config_template') . '/template/module/tmdsociallogin.tpl', $data);
		} else {
			return $this->load->view('default/template/module/tmdsociallogin.tpl', $data);
		}
		}
	...
	public function fbredirecturl() {
		...
		if(!isset($this->fbconnect)){
			...
			require_once(DIR_SYSTEM . 'library/tmdsocial/facebook.php');
	...
	public function likinredirect() {
		...
		if(isset($this->session->data['route']))
		{
				if($this->session->data['route']=='common/quicklogin')
				{
					$this->request->get['route']='account/account';
				}
