...
class ControllerModuleShowcase extends Controller {
	public function index($setting) {
		...
		if (isset($setting['showcase'][$this->config->get('config_language_id')]['title'])) {
			$data['title'] = html_entity_decode($setting['showcase'][$this->config->get('config_language_id')]['title'], ENT_QUOTES, 'UTF-8');
		} else {
			$data['title'] = '';
		}

		$data['tag'] = isset($setting['showcase'][$this->config->get('config_language_id')]['tag']) ? $setting['showcase'][$this->config->get('config_language_id')]['tag'] : 'h3';
		...
		$data['autowidth'] = !empty($setting['showcase']['autowidth']) ? $setting['showcase']['autowidth'] : '';
		$data['subautowidth'] = !empty($setting['showcase']['subautowidth']) ? $setting['showcase']['subautowidth'] : '';

		$data['scrolltop'] = !empty($setting['showcase']['scrolltop']) ? $setting['showcase']['scrolltop'] : '';
		$data['scrolltop_speed'] = !empty($setting['showcase']['scrolltop_speed']) ? (int)$setting['showcase']['scrolltop_speed'] : '300';
					...
					if ($data['subitems_status']) {
						...
						if ($data['products_by_item']) {
							...
							foreach($subitems as $subitem) {
                                ...
                                $the_name = $subitem['sc_name'] ? $subitem['sc_name'] : $subitem['name'];
                                $subitems_data[] = array(
									'name'        => $the_name,
									'href'        => $href,
			...
			foreach ($items as $item) {
				...
				if ($item) {
					...
					$data['items'][] = array(
						...
						'item_sd'     => !empty($item['sc_description']) ? html_entity_decode($item['sc_description'], ENT_QUOTES, 'UTF-8') : false,
						'active'      => false,
						'description' => false,
						'count'       => false
						);
