    ...
    public function view_module_feautured_before(&$view, &$data, &$output)
    {
        ...
        if(!empty($data['products'])){
            foreach ($data['products'] as $key => $value) {
                ...
                $data['products'][$key]['description'] = $this->{'model_module_'.$this->codename}->getText($product_info['description']);
                if(VERSION>='2.2.0.0')
                {
                ...
                {
                    $data['products'][$key]['description'] = utf8_substr(strip_tags(html_entity_decode($data['products'][$key]['description'], ENT_QUOTES, 'UTF-8')), 0, $this->config->get('config_product_description_length')) . '..';
                }
    ...
    public function model_getProducts_after(&$route, &$data, &$output)
    {
        if(!empty($output)){
            foreach ($output as $key => $value) {
                $output[$key]['description'] = $this->{'model_module_'.$this->codename}->getText($value['description']);
            }
        }
    }
